/**
  * Auth Failed — Experiment (cloned from ConnectorSkills P0)
 *
...
const SRC_CONNECTORS: SrcConn[] = [
  ...
  { logo: 'lseg-logo.png', name: 'London Stock Exchange Group', desc: 'Market data, filings, and company fundamentals', trailing: 'authfail', cat: 'Financial Services' },
...
function responseBlocksLsegHtml(): string {
  const foot = (icon: string, label: string) => '<button class="rfoot__btn" data-tip="' + label + '" title="' + label + '" aria-label="' + label + '">' + ico(icon) + '</button>';
  const toggleFoot = (idName: string, reg: string, fil: string, label: string) => '<button class="rfoot__btn rfoot__btn--toggle" id="' + idName + '" data-tip="' + label + '" title="' + label + '" aria-label="' + label + '"><span class="ico-r">' + ico(reg) + '</span><span class="ico-f">' + ico(fil) + '</span></button>';
  const lsegCite = (plus: number) => '<span class="cite cite--web">' + ico('globe-20-regular') + '<span class="cite__name">LSEG</span>' + (plus ? '<span class="cite__plus">+' + plus + '</span>' : '') + '</span>';
  return [
    '<button class="cotbar" id="cotBar" aria-expanded="false"><span class="cotbar__mark">' + ico('copilot-20-regular') + '</span><span class="cotbar__label">Reasoned about Microsoft FY26 broker estimates</span><span class="cotbar__chev">' + ico('chevron-right-16-regular') + '</span></button>',
    '<p class="resp__p">Based on consensus broker estimates aggregated by <a class="resp__link" data-ref="0" onclick="return false" href="#">LSEG</a>, analysts project Microsoft\u2019s FY26 net income at roughly $118 billion \u2014 up about 12% year over year, with Azure and Copilot monetisation the main drivers. ' + lsegCite(2) + '</p>',
    '<p class="resp__p">The mean estimate across 34 contributing brokers is $118.4B (range $110B\u2013$126B), implying EPS of about $15.40. Revisions over the past 90 days have trended upward on stronger cloud growth. ' + lsegCite(0) + '</p>',
    '<hr class="resp__hr"/>',
    '<h4 class="resp__h4">What\u2019s moving the estimates</h4>',
    '<p class="resp__p">Upside cases center on AI\u2011attached ARPU and Azure acceleration; downside cases flag elevated capex and FX headwinds.</p>',
    '<div class="rfoot">',
    '<div class="rfoot__toolbar">',
    foot('copy-20-regular', 'Copy'),
    toggleFoot('rfLike', 'thumb-like-20-regular', 'thumb-like-20-filled', 'Good response'),
    ...
    '<div class="rfoot__divider"></div>',
    '<button class="rfoot__refs" id="rfRefs" data-ref="all" aria-label="References"><span class="rfoot__avatars">'
      + '<span class="rfoot__avatar" style="background:#1a12ff;color:#fff;font-weight:700">L</span>'
      + '<span class="rfoot__avatar rfoot__avatar--r">R</span>'
      + '</span><span class="rfoot__refs-label">References</span></button>',
    '</div>',
  ].join('');
}
...
.rcx { display: flex; flex-direction: column; max-width: 720px; animation: respIn 0.28s ease both; }
.rcx__msg { font-size: 16px; line-height: 28px; font-weight: 420; color: #242424; }
.rcx__actions { display: flex; align-items: center; gap: 8px; margin-top: 16px; }
.rcx__btn { display: inline-flex; align-items: center; gap: 10px; height: 40px; padding: 0 16px 0 8px; border: 1px solid #dedede; border-radius: 10px; background: #fff; font-family: inherit; font-size: 14px; line-height: 20px; font-weight: 625; font-variation-settings: 'opsz' 8, 'wght' 625; color: #242424; cursor: pointer; transition: background 0.1s, border-color 0.1s; }
.rcx__btn:hover { background: #f5f5f5; border-color: #c4c4c4; }
.rcx__logo { display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; border-radius: 6px; overflow: hidden; flex-shrink: 0; }
.rcx__logo img { width: 24px; height: 24px; object-fit: contain; display: block; }
.rcx__skip { border: none; background: none; font-family: inherit; font-size: 14px; line-height: 20px; font-weight: 420; color: #242424; cursor: pointer; padding: 8px 12px; border-radius: 8px; transition: background 0.1s; }
.rcx__skip:hover { background: rgba(36,36,36,0.04); }
.lseg3p { position: fixed; inset: 0; z-index: 600; background: #d4d4d4; display: flex; align-items: center; justify-content: center; font-family: Arial, 'Segoe UI', sans-serif; transform: translateX(100%); visibility: hidden; transition: transform 0.42s cubic-bezier(0.22,0.61,0.36,1), visibility 0s linear 0.42s; }
.lseg3p.is-open { transform: translateX(0); visibility: visible; transition: transform 0.42s cubic-bezier(0.22,0.61,0.36,1), visibility 0s; }
@media (prefers-reduced-motion: reduce) { .lseg3p { transition: none; } }
.lseg3p__card { width: 294px; background: #fff; display: flex; flex-direction: column; box-shadow: 0 1px 6px rgba(0,0,0,0.12); }
.lseg3p__body { padding: 14px 24px 20px; display: flex; flex-direction: column; }
.lseg3p__lang { font-size: 12px; color: #333; display: inline-flex; align-items: center; gap: 4px; }
.lseg3p__brand { display: flex; align-items: center; justify-content: center; margin: 22px 0 30px; }
.lseg3p__brand span { font-size: 34px; font-weight: 800; color: #1a0dcc; letter-spacing: 1px; }
.lseg3p__label { font-size: 13px; font-weight: 700; color: #222; margin-bottom: 5px; }
.lseg3p__field { display: flex; align-items: center; border: 1px solid #b3b3b3; height: 38px; padding: 0 10px; }
.lseg3p__field input { flex: 1; min-width: 0; border: none; outline: none; font-size: 13px; color: #333; background: none; font-family: inherit; }
.lseg3p__ico { display: inline-flex; flex-shrink: 0; }
.lseg3p__link { color: #1a0dcc; font-size: 12px; margin: 6px 0 16px; text-decoration: none; cursor: pointer; }
.lseg3p__link:hover { text-decoration: underline; }
.lseg3p__actions { display: flex; justify-content: flex-end; gap: 8px; margin-top: 8px; margin-bottom: 20px; }
.lseg3p__back { background: #f2f2f2; border: 1px solid #ccc; color: #222; font-size: 12px; font-weight: 700; padding: 9px 18px; cursor: pointer; font-family: inherit; }
.lseg3p__back:hover { background: #e8e8e8; }
.lseg3p__signin { background: #000; border: 1px solid #000; color: #fff; font-size: 12px; font-weight: 700; padding: 9px 18px; cursor: pointer; font-family: inherit; }
.lseg3p__signin:hover { background: #222; }
.lseg3p__footer { background: #1a12ff; color: #fff; font-size: 12px; padding: 8px 16px; display: flex; gap: 10px; align-items: center; }
.lseg3p__footer a { color: #fff; text-decoration: none; }
.lseg3p__footer a:hover { text-decoration: underline; }
.lseg3p__sep { opacity: 0.7; }
...
.srcm__warntip { display: none; position: absolute; bottom: calc(100% + 10px); left: 50%; transform: translateX(-50%); width: max-content; max-width: 240px; background: #fff; color: #5d5d5d; font-family: 'Segoe Sans','Segoe UI',sans-serif; font-size: 12px; line-height: 16px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; padding: 10px 12px; border-radius: 12px; text-align: left; filter: drop-shadow(0 2px 8px rgba(0,0,0,0.14)); opacity: 0; pointer-events: none; transition: opacity 0.12s ease, transform 0.12s ease; z-index: 400; }
.srcm__warntip::after { content: ''; position: absolute; top: 100%; left: 50%; width: 8.485px; height: 8.485px; background: #fff; margin: -5.243px 0 0 -4.243px; transform: rotate(45deg); }
.srcm__warn:hover .srcm__warntip, .srcm__warn:focus-visible .srcm__warntip { opacity: 1; }
.stgd__msgbar { display: flex; align-items: flex-start; gap: 8px; padding: 12px; border: 1px solid #ffd3c4; border-radius: 16px; background: #fff2ee; margin-bottom: 4px; }
.stgd__msgbar__ico { color: #a93901; flex-shrink: 0; display: inline-flex; }
.stgd__msgbar__ico svg { width: 20px; height: 20px; }
.stgd__msgbar__body { flex: 1; min-width: 0; font-size: 14px; line-height: 20px; font-weight: 420; color: #242424; }
.stgd__msgbar__actions { display: inline-flex; align-items: center; gap: 4px; flex-shrink: 0; }
.stgd__msgbar__btn { border: none; background: transparent; font-family: inherit; font-size: 14px; line-height: 20px; font-weight: 420; color: #242424; padding: 4px 8px; border-radius: 8px; cursor: pointer; }
.stgd__msgbar__btn:hover { background: rgba(36,36,36,0.06); }
...
.stgd__logo { width: 24px; height: 24px; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; border-radius: 6px; overflow: hidden; }
.stgd__logo img { width: 24px; height: 24px; object-fit: contain; display: block; }
.stgd__logo .stg__gi { width: 24px; height: 24px; }
.stgd__logo svg { width: 24px; height: 24px; }
...
.sk-pop { position: fixed; z-index: 320; width: 260px; filter: drop-shadow(0 2px 8px rgba(0,0,0,0.14)); opacity: 0; transform: translateY(2px); transition: opacity 0.12s ease, transform 0.12s ease; pointer-events: none; }
...
const js = [
  ...
  "  var RESP_LSEG_HTML = " + JSON.stringify('<div class="resp" id="respBlk">' + responseBlocksLsegHtml() + '</div>') + ";",
  ...
  "  var LSEG_QUERY = " + JSON.stringify('As per market data, what are broker estimates for Microsoft net income in FY26?') + ";",
  "  var RECONNECT_HTML = " + JSON.stringify('<div class="rcx"><div class="rcx__msg">Your LSEG connection has expired. Please reconnect to include LSEG content in this response.</div><div class="rcx__actions"><button class="rcx__btn" id="rcxReconnect"><span class="rcx__logo"><img src="' + logoImg('lseg-logo.png') + '" alt=""/></span>Reconnect LSEG</button><button class="rcx__skip" id="rcxSkip">Skip</button></div></div>') + ";",
  ...
  "  function sendMessage(){ if (!ta || !thread) return; var text = ta.value.trim(); if (!text) return; shellEl.classList.add('shell--convo'); thread.innerHTML = U_OPEN + escapeHtml(text) + U_CLOSE; var cw = thread.querySelector('.cwrap'); if (cw) cw.innerHTML = THINK_HTML; ta.value = ''; if (cmp) cmp.classList.remove('ci--has-text'); thread.scrollTop = thread.scrollHeight; var _lseg = SRC.filter(function(x){ return x.n === 'London Stock Exchange Group'; })[0]; var _needReconnect = _lseg && _lseg.state === 'authfail'; if (!_needReconnect) shellEl.classList.add('shell--sp-open'); if (respTimer) clearTimeout(respTimer); respTimer = setTimeout(function(){ var w = thread.querySelector('.cwrap'); if (w){ if (_needReconnect){ w.innerHTML = RECONNECT_HTML; bindReconnectPrompt(); } else { w.innerHTML = RESP_HTML; var r = document.getElementById('respBlk'); requestAnimationFrame(function(){ if (r) r.classList.add('is-in'); }); bindResp(); } } thread.scrollTop = thread.scrollHeight; }, 1500); }",
  "  if (sendBtn) sendBtn.addEventListener('click', sendMessage);",
  "  if (ta) ta.addEventListener('keydown', function(e){ if (e.key === 'Enter' && !e.shiftKey){ e.preventDefault(); sendMessage(); } });",
  "  function bindReconnectPrompt(){ var rb = document.getElementById('rcxReconnect'); var sk = document.getElementById('rcxSkip'); if (rb) rb.addEventListener('click', function(){ var it = SRC.filter(function(x){ return x.n === 'London Stock Exchange Group'; })[0]; if (typeof openAuth === 'function') openAuth('London Stock Exchange Group', it ? it.logoSrc : ''); }); if (sk) sk.addEventListener('click', function(){ var w = thread.querySelector('.cwrap'); if (!w) return; w.innerHTML = RESP_HTML; var r = document.getElementById('respBlk'); requestAnimationFrame(function(){ if (r) r.classList.add('is-in'); }); bindResp(); shellEl.classList.add('shell--sp-open'); thread.scrollTop = thread.scrollHeight; }); }",
  "  if (ta){ ta.value = LSEG_QUERY; if (typeof syncSend === 'function') syncSend(); }",
  "  var lsegAuthOv = document.getElementById('lsegAuth'); var lsegAuthCtx = null;",
  "  function openLsegAuth(ctx){ lsegAuthCtx = ctx || 'ui'; if (lsegAuthOv) lsegAuthOv.classList.add('is-open'); }",
  "  function hideLsegAuth(){ if (lsegAuthOv) lsegAuthOv.classList.remove('is-open'); }",
  "  function lsegReconnected(){ var it = SRC.filter(function(x){ return x.n === 'London Stock Exchange Group'; })[0]; if (it) it.state = 'on'; if (typeof srcPaint === 'function') srcPaint(); if (typeof stgPaint === 'function') stgPaint(); if (typeof updateSrcCount === 'function') updateSrcCount(); if (typeof agentsPaintSources === 'function') agentsPaintSources(); try { if (currentStgName === 'London Stock Exchange Group'){ var _ov = document.getElementById('stgOverlay'); if (_ov && _ov.classList.contains('is-open') && typeof openStgDetail === 'function') openStgDetail('London Stock Exchange Group'); } } catch(e){} var ctx = lsegAuthCtx; hideLsegAuth(); if (ctx === 'chat' && thread){ var w = thread.querySelector('.cwrap'); if (w){ w.innerHTML = THINK_HTML; shellEl.classList.add('shell--sp-open'); thread.scrollTop = thread.scrollHeight; if (respTimer) clearTimeout(respTimer); respTimer = setTimeout(function(){ var w2 = thread.querySelector('.cwrap'); if (w2){ w2.innerHTML = RESP_LSEG_HTML; var r = document.getElementById('respBlk'); requestAnimationFrame(function(){ if (r) r.classList.add('is-in'); }); bindResp(); } thread.scrollTop = thread.scrollHeight; }, 1500); } } }",
  "  (function(){ var sb = document.getElementById('lsegSignIn'); if (sb) sb.addEventListener('click', lsegReconnected); var bk = document.getElementById('lsegBack'); if (bk) bk.addEventListener('click', hideLsegAuth); })();",
  "  document.addEventListener('click', function(e){ var t = e.target.closest && e.target.closest('.srcm__reconnect, .stg__reconnect, #stgdReconnectBtn, #rcxReconnect'); if (!t) return; if (t.classList.contains('srcm__reconnect--busy') || t.classList.contains('stg__reconnect--busy') || t.classList.contains('stgd__reconnect--busy')) return; e.preventDefault(); e.stopImmediatePropagation(); openLsegAuth(t.id === 'rcxReconnect' ? 'chat' : 'ui'); }, true);",
  ...
  "  document.addEventListener('mouseover', function(e){ var wp = e.target.closest && e.target.closest('.srcm__warn'); if (!wp) return; var desc = wp.getAttribute('aria-label'); if (!desc) return; skPopBody.textContent = desc; skPop.classList.add('is-visible'); var r = wp.getBoundingClientRect(); var pw = skPop.offsetWidth, ph = skPop.offsetHeight; var cx = r.left + r.width / 2; var left = cx - pw / 2; if (left + pw > window.innerWidth - 8) left = window.innerWidth - 8 - pw; if (left < 8) left = 8; var above = true; var top = r.top - ph - 10; if (top < 8){ above = false; top = r.bottom + 10; } skPop.classList.toggle('sk-pop--above', above); skPop.classList.toggle('sk-pop--below', !above); skPop.style.left = left + 'px'; skPop.style.top = top + 'px'; var bx = cx - left; bx = Math.max(16, Math.min(pw - 16, bx)); skPopBeak.style.left = bx + 'px'; });",
  "  document.addEventListener('mouseout', function(e){ var wp = e.target.closest && e.target.closest('.srcm__warn'); if (!wp) return; if (e.relatedTarget && wp.contains(e.relatedTarget)) return; skPop.classList.remove('is-visible'); });",
  "  document.addEventListener('copilot:variant-change', function(e){ var d = e.detail || {}; if (d.group && d.group.indexOf('lsegWarn') === 0){ var stgD = document.getElementById('stgDetail'); if (!stgD) return; var wi = stgD.querySelector('.stgd__namewrap .srcm__warn'); if (wi) wi.style.display = (d.variant === 'message') ? 'none' : ''; } });",
  ...
  "  function srcRowHtml(item){ var meta = item.meta ? '<span class=\"srcm__meta\">' + item.meta + '</span>' : ''; var recBadge = (item.rec && item.state === 'connect') ? '<span class=\"badge\">Recommended</span>' : ''; var newBadge = (item.badge && item.state === 'connect') ? '<span class=\"badge badge--brand\">New</span>' : ''; var warn = item.state === 'authfail' ? '<span class=\"srcm__warn\" tabindex=\"0\" aria-label=\"Your LSEG signin has expired. Please reconnect the source.\">' + WARN_ICON + '<span class=\"srcm__warntip\" role=\"tooltip\">Your LSEG signin has expired. Please reconnect the source.</span></span>' : ''; return '<div class=\"srcm__row\" data-name=\"' + item.n + '\"><div class=\"srcm__inner\">' + item.logo + '<span class=\"srcm__label\">' + item.n + '</span>' + warn + recBadge + newBadge + '<span class=\"srcm__spacer\"></span>' + meta + srcTrailing(item) + '</div></div>'; }",
...
// LSEG third-party sign-in page (shown when the user reconnects an expired source)
function lsegAuthPage(): string {
  ...
  return [
    '<div class="lseg3p" id="lsegAuth">',
    '<div class="lseg3p__card">',
    '<div class="lseg3p__body">',
    '<div class="lseg3p__lang">English ' + CHEV + '</div>',
    '<div class="lseg3p__brand"><span>LSEG</span></div>',
    '<label class="lseg3p__label">User ID</label>',
    '<div class="lseg3p__field"><input type="text" placeholder="dfew" aria-label="User ID"/><span class="lseg3p__ico">' + PERSON + '</span></div>',
    '<a class="lseg3p__link" href="#" onclick="return false">Forgotten User ID?</a>',
    '<label class="lseg3p__label">Password</label>',
    '<div class="lseg3p__field"><input type="password" placeholder="Enter password" aria-label="Password"/><span class="lseg3p__ico">' + EYE + '</span></div>',
    '<a class="lseg3p__link" href="#" onclick="return false">Forgotten password?</a>',
    '<div class="lseg3p__actions"><button class="lseg3p__back" id="lsegBack">BACK</button><button class="lseg3p__signin" id="lsegSignIn">SIGN IN</button></div>',
    '</div>',
    '<div class="lseg3p__footer"><a href="#" onclick="return false">Contact Us</a><span class="lseg3p__sep">|</span><a href="#" onclick="return false">Privacy</a></div>',
    '</div>',
    '</div>',
  ].join('');
}

const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Auth Failed</title><style>' + css + agentsCss() + '</style></head><body>'
  + '<div class="shell">'
  + nav()
...
fs.writeFileSync(path.join(outDir, 'authFailed.html'), html, 'utf-8');
console.log('Done: ' + path.join(outDir, 'authFailed.html'));
