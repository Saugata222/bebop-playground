/**
 * Copilot Shell — Experiment (cloned from ConnectorSkills P0)
 *
...
function responseBlocksHtml(): string {
  ...
  return [
    '<button class="cotbar" id="cotBar" aria-expanded="false"><span class="cotbar__mark">' + ico('copilot-20-regular') + '</span><span class="cotbar__label">Reasoning completed in 6 steps</span><span class="cotbar__chev">' + ico('chevron-right-16-regular') + '</span></button>',
...
const portfolioPrompt = 'Help me analyse my client\u2019s investment portfolio against current market trends. Identify the stocks to hold or sell.';
const portfolioRemoveIcon = '<svg viewBox="0 0 12 12" width="12" height="12" fill="none"><path d="M3 3l6 6M9 3l-6 6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>';
const portfolioAttachment = '<div class="cmp-attach" data-file="John Portfolio FY26" title="John Portfolio FY26.xlsx">'
  ...
  + '<button class="cmp-attach__x" aria-label="Remove John Portfolio FY26">' + portfolioRemoveIcon + '</button>'
  + '</div>';
...
const js = [
  ...
  "  var REASON_STEPS = ['Reading attached file', 'Searching market data', 'Connecting to LSEG', 'Pulling live quotes and fundamentals', 'Assessing market trends', 'Generating recommendations'];",
  ...
  "  var invocationPrompt = " + JSON.stringify(portfolioPrompt) + "; var invocationAttachHtml = " + JSON.stringify(portfolioAttachment) + ";",
  ...
  "  agentsPaintSources();",
  "  tintSourceCardIcons();",
  "  var initialView = null; (function(){ try { var query = new URLSearchParams(location.search); var view = query.get('view'); initialView = view; if (view && nav) nav.classList.add('nav--collapsed'); if (view === 'settings' && stgOv){ stgOv.classList.add('is-open'); if (typeof stgPaint === 'function') stgPaint(); } else if (view === 'ciq'){ if (ta){ ta.value = 'Draft a research proposal based on '; try { ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); } catch(e){} } if (typeof syncSend === 'function') syncSend(); } else if (view === 'implicit'){ resetInvocation(); } else if (view === 'disc'){ RESP_HTML = RESP_HTML_DISC; REASON_STEPS = REASON_STEPS_DISC; var cotList = document.querySelector('#spPanel .cot-list'); if (cotList) cotList.innerHTML = COT_DISC_STEPS; resetInvocation(); } } catch(e){} })();",
...
const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Copilot Shell</title><style>' + css + agentsCss() + '</style></head><body>'
  + '<div class="shell">'
  + nav()
...
fs.writeFileSync(path.join(outDir, 'copilotShell.html'), html, 'utf-8');
console.log('Done: ' + path.join(outDir, 'copilotShell.html'));
