/**
 * One Copilot Shell — Full-Screen Interactive Shell (Work)
 *
 * A full-viewport assembly of the One Copilot compounds with the connector +
 * skills experience layered in. Composes the Side Nav (collapsible, with the
 * Segmented Switch pivot), Header (Commercial), Greeting & Composer zero state,
 * Prompt Lab, and the Me menu, plus:
 *   - Connector skills: per-source "/skill" procedures (from preview/src/_skills.ts),
 *     a chevron on connected sources in Change-sources -> a skills sub-view, a
 *     Skills section on the Settings L2 source detail, and skill hover popovers.
 *   - CIQ palette below the composer ("/" trigger) + the "Add content" modal,
 *     with skill attachment pills in the composer.
 *   - Settings L2 source detail (about, tools, admin, reconnect, disconnect).
 *   - Auth flow: USC input auth, SSO login redirect, disconnect confirmation,
 *     and sync / removing timers.
 *   - Animated thinking hero + a cycling multi-step reasoning stream.
 *
 * Only the connected-source response variant is shown. Four sources are
 * connected by default (Microsoft 365 apps, London Stock Exchange Group,
 * Confluence, Jira) and the composer starts empty.
 *
 * Cloned from ConnectorSkills P0 (shared connector-source-shell family); the
 * Agents & Skills store view, the disconnected response variant, and the
 * variants runtime are intentionally omitted.
 *
...
// Animated Copilot thinking mark (Figma ThinkingState_Hero) inlined as a data URI so the dist stays self-contained.
const THINKING_HERO = (function(){ try { return 'data:image/gif;base64,' + fs.readFileSync(path.join(iconsDir, 'thinking-hero.gif')).toString('base64'); } catch (e) { return ''; } })();
...
function brandSvg(name: string): string {
  try {
    ...
    // Strip width/height only from the opening <svg> tag; inner <rect>/<g> sizes must survive.
...
function nav(): string {
  return [
    ...
    item(ico('agents-20-regular'), 'Agents &amp; Skills', { id: 'agentsNavBtn' }),
...
interface SrcConn { logo: string; name: string; meta?: string; desc: string; trailing: 'on' | 'off' | 'connect' | 'chevron' | 'authfail'; tenant?: boolean; sync?: boolean; cat: string; badge?: boolean; usc?: boolean; uscHost?: string; uscDev?: string; uscAdd?: string; uscPh?: string; }
const SRC_CONNECTORS: SrcConn[] = [
  ...
  { logo: 'servicenow-logo.png', name: 'ServiceNow', desc: 'Look up tickets, incidents, and IT service records', trailing: 'connect', cat: 'Development tools' },
  { logo: 'azure-devops-logo.svg', name: 'Azure DevOps', desc: 'Reference repos, pipelines, boards, and work items', trailing: 'connect', cat: 'Development tools' },
  ...
  { logo: 'canva-logo.png', name: 'Canva', desc: 'Browse, summarize, and generate new Canva designs', trailing: 'connect', cat: 'Creative' },
  ...
  { logo: 'jira-logo.png', name: 'Jira', desc: 'Track issues, sprints, boards, and project workflows', trailing: 'chevron', cat: 'Development tools' },
  { logo: 'confluence-logo.png', name: 'Confluence', desc: 'Pull knowledge base articles and team documentation', trailing: 'chevron', cat: 'Development tools', usc: true, uscHost: 'atlassian.net', uscDev: 'Atlassian' },
  ...
  { logo: 'lseg-logo.png', name: 'London Stock Exchange Group', desc: 'Market data, filings, and company fundamentals', trailing: 'chevron', cat: 'Financial Services' },
...
const SRC_MODEL = SRC_CONNECTORS.map(function (c) {
  return { n: c.name, logo: srcLogoHtml(c.logo), stgLogo: stgLogoHtml(c.logo), logoSrc: c.logo.endsWith('.svg') ? '' : logoImg(c.logo), meta: c.meta || '', desc: c.desc, state: c.trailing, cats: c.cat ? [c.cat] : [], rec: SRC_REC.indexOf(c.name) !== -1, tenant: c.tenant === true, sync: c.sync === true, badge: c.badge === true, usc: c.usc === true, uscHost: c.uscHost || '', uscDev: c.uscDev || '', uscAdd: c.uscAdd || '', uscPh: c.uscPh || '' };
});
...
const RICH_SKILLS: Record<string, Skill[]> = {
  ...
  'London Stock Exchange Group': [
    { name: 'company-brief', desc: 'Summarize a company\u2019s market profile, fundamentals, and recent activity.' },
    { name: 'earnings-recap', desc: 'Recap the latest earnings release and key investor takeaways.' },
    { name: 'peer-benchmark', desc: 'Benchmark a company against its sector peers.' },
    { name: 'filings-digest', desc: 'Summarize recent regulatory filings and material changes.' },
    { name: 'valuation-snapshot', desc: 'Compare valuation multiples with historical and peer ranges.' },
    { name: 'market-summary', desc: 'Summarize today\u2019s market movers and drivers.' },
    { name: 'ownership-changes', desc: 'Summarize recent institutional ownership changes.' },
    { name: 'watchlist-update', desc: 'Report the latest prices, filings, and events for a watchlist.' },
    { name: 'dividend-history', desc: 'Show dividend payments and changes over time.' },
  ],
  'S&P Global': [
...
SRC_CONNECTORS.forEach(function (c) {
  SRC_SKILLS[c.name] = c.name === 'Microsoft 365 apps' ? [] : (RICH_SKILLS[c.name] || skillsFor(c.name).map(function (sk) { return { name: sk.name, desc: sk.desc }; }));
});
const CIQ_SKILL_EXCLUDE = ['ServiceNow', 'Azure DevOps'] as const;
...
const CIQ_FILES = [
  ...
  { icon: brandSvg('excel-app-20-color'), title: 'FY25 Revenue model', sub: 'Opened last week', kind: 'content' },
];
const CIQ_PEOPLE = [
...
const CIQ_TABS = [
  ...
  { id: 'skills', label: 'Agents & skills', rows: CIQ_SKILLS },
...
const CIQ_MTABS = [
  ...
  { id: 'skills', label: 'Agents & skills', rows: [] },
  { id: 'files', label: 'Files', rows: CIQ_FILES },
  { id: 'people', label: 'People', rows: CIQ_PEOPLE },
  { id: 'meetings', label: 'Meetings', rows: CIQ_MEETINGS },
  { id: 'emails', label: 'Emails', rows: CIQ_EMAILS },
...
const SRC_DETAIL: Record<string, { about: string; tools: string[]; developer: string; category?: string; version?: string; connectedOn?: string }> = {
  'Microsoft 365 apps': {
  ...
  'Azure DevOps': {
    about: 'Track work items and pull requests from your Azure DevOps organisations. View boards, manage pipelines, and link commits directly within Copilot.',
...
function sourcesModal(): string {
  return [
    ...
    '<div class="srcm__search">' + ico('search-20-regular') + '<input id="srcSearch" placeholder="Search for a source or category" aria-label="Search sources"/></div>',
    '<div id="srcMainView">',
...
function authModal(): string {
  return [
    ...
    '<button class="ad__close ad__close--abs" id="adCancel" aria-label="Close">' + ico('dismiss-20-regular') + '</button>',
    '<div class="ad__hand">',
    '<span class="ad__cop">' + ico('copilot-20-regular') + '</span>',
    '<span class="ad__dots">' + ico('more-horizontal-20-regular') + '</span>',
    '<span class="ad__logo ad__logo--lg" id="adPartner"></span>',
    '</div>',
    '<div class="ad__text">',
    '<h2 class="ad__name" id="adName2">Connect</h2>',
    '<p class="ad__body" id="adBody"></p>',
    '</div>',
    '<div class="ad__meta">',
    '<div class="ad__metacol"><span class="ad__metal">Developed by</span><span class="ad__metav" id="adDev">\u2014</span></div>',
    '<div class="ad__metacol"><span class="ad__metal">MCP Endpoint</span><span class="ad__metav" id="adMcp">\u2014</span></div>',
    '<div class="ad__metacol"><span class="ad__metal">More info</span><span class="ad__metav"><a class="ad__link" href="#" onclick="return false">Privacy Policy</a>, <a class="ad__link" href="#" onclick="return false">Terms of Service</a></span></div>',
    '</div>',
    '<div class="ad__rule"></div>',
...
// ─── Third-party login / redirect page (shown after Continue) ─────────
function ssoLoginPage(): string {
  const LOCK = '<svg width="14" height="14" viewBox="0 0 20 20" fill="none"><path d="M6 8V6.5a4 4 0 118 0V8h.4A1.6 1.6 0 0116 9.6v5.9A1.6 1.6 0 0114.4 17H5.6A1.6 1.6 0 014 15.5V9.6A1.6 1.6 0 015.6 8H6zm1.5 0h5V6.5a2.5 2.5 0 00-5 0V8z" fill="#616161"/></svg>';
  return [
    '<div class="sso" id="ssoLogin">',
    '<div class="sso__card">',
    '<div class="sso__body">',
    '<div class="sso__brand"><span class="sso__logo" id="ssoLogo"></span><span class="sso__name" id="ssoName">Source</span></div>',
    '<div class="sso__title" id="ssoTitle">Sign in</div>',
    '<label class="sso__label">Email or username</label>',
    '<div class="sso__field"><input type="text" id="ssoUser" placeholder="name@company.com" aria-label="Email or username"/></div>',
    '<label class="sso__label">Password</label>',
    '<div class="sso__field"><input type="password" id="ssoPass" placeholder="Enter password" aria-label="Password"/></div>',
    '<a class="sso__link" href="#" onclick="return false">Forgot password?</a>',
    '<div class="sso__actions"><button class="sso__cancel" id="ssoCancel">Cancel</button><button class="sso__signin" id="ssoSignin">Sign in</button></div>',
    '</div>',
    '<div class="sso__foot"><span class="sso__lock">' + LOCK + '</span><span>You\u2019re being securely redirected by Microsoft Copilot</span></div>',
    '</div>',
    '</div>',
  ].join('');
}
...
function cotStep(label: string, body?: string, subs?: readonly string[], open?: boolean): string {
  const bodyHtml = (body || (subs && subs.length))
    ? '<div class="cot-step__body">'
        + (body ? '<p>' + body + '</p>' : '')
        + (subs && subs.length ? '<ul class="cot-subs">' + subs.map(function (s) { return '<li>' + s + '</li>'; }).join('') + '</ul>' : '')
      + '</div>'
    : '';
  return '<div class="cot-item' + (open ? ' is-open' : '') + '"><button class="cot-step"><span class="cot-step__chev">' + ico('chevron-right-16-regular') + '</span><span class="cot-step__label">' + label + '</span></button>' + bodyHtml + '</div>';
}
function srch(...queries: string[]): string {
  return cotStep('Searches', undefined, queries);
}
function cotPanel(): string {
  return [
    ...
    '<div class="sp__body"><div class="cot-list">',
    cotStep('Reading attached file', 'Scanning the workbook to understand the holdings before pulling any market data.', [
      'Opened John Portfolio FY26.xlsx and read the holdings tab',
      'Identified positions: AMZN, NVDA, META and cash',
      'Framed the goal: recommend hold vs. sell against current trends',
    ]),
    cotStep('Searching market data', 'Gathering the current market context to frame the analysis.', [
      'Searched LSEG Workspace \u2014 current equity market overview',
      'Searched Web \u2014 2026 market outlook and rate expectations',
      'Reviewed S&P 500 sector performance year to date',
    ]),
    cotStep('Searching for finance skills', 'Looking for the right finance skill to ground the analysis before pulling data.', [
      'Scanned connected plugins for finance capabilities',
      'Matched the request to the Market Researcher skill',
      'Shortlisted skills that can read quotes and fundamentals',
    ]),
    cotStep('Loading market researcher skill', 'Loading the Market Researcher skill and preparing its tools.', [
      'Loaded the Market Researcher skill',
      'Registered its LSEG-backed market data tools',
      'Readied the skill to fetch live quotes and fundamentals',
    ]),
    cotStep('Connecting to LSEG', 'Establishing a live connection to LSEG Workspace for quotes and fundamentals.', [
      'Connected to LSEG Workspace and confirmed data access',
      'Located quote and fundamentals endpoints for AMZN, NVDA, META',
      'Confirmed no additional tickers needed beyond the workbook',
    ]),
    cotStep('Pulling live quotes and fundamentals', 'Collecting live prices, fundamentals, and analyst consensus for each holding.', [
      'Retrieved live quotes for AMZN, NVDA, META',
      'Pulled key fundamentals: P/E, revenue growth, margins',
      'Collected analyst consensus ratings and price targets',
      'Read LSEG close prices, timestamped to the latest market close',
    ]),
    cotStep('Assessing market trends', 'Weighing signals per holding and setting the guardrails for the answer.', [
      'Weighed price momentum, analyst consensus, and sector trend',
      'Reviewed semiconductor and cloud demand trends for 2026',
      'Added a clear \u201Cnot a financial advisor\u201D disclaimer, keeping recommendations informational',
    ]),
    cotStep('Generating recommendations', 'Computing the numbers and citing the sources behind each figure.', [
      'Market value = shares \u00D7 current price, per position',
      'Unrealized gain/loss = market value \u2212 cost basis; rolled up portfolio total and weights',
      'Produced a hold/sell recommendation for each position',
      'Cited LSEG Workspace quotes and John Portfolio FY26.xlsx behind each figure',
    ], true),
    '</div></div>',
    '</aside>',
  ].join('');
...
function citeFile(title: string, refs: string, plus?: number): string {
  return '<span class="cite cite--file cite--excel" data-refs="' + refs + '" title="' + title + '">' + brandSvg('excel-app-20-color') + (plus ? '<span class="cite__plus">+' + plus + '</span>' : '') + '</span>';
}
const LSEG_MARK = '<svg viewBox="0 0 20 20" width="14" height="14" xmlns="http://www.w3.org/2000/svg"><rect width="20" height="20" rx="4" fill="#5d5d5d"/><text x="10" y="13.6" text-anchor="middle" font-family="Segoe UI, Arial, sans-serif" font-size="7.2" font-weight="700" letter-spacing="-0.3" fill="#ffffff">LSEG</text></svg>';
const LSEG_LOGO_IMG = '<img src="' + logoImg('lseg-logo.png') + '" alt="LSEG"/>';
function citeWeb(name: string, refs: string, plus?: number): string {
  const mark = /^LSEG/.test(name) ? LSEG_MARK : ico('globe-20-regular');
  return '<span class="cite cite--web" data-refs="' + refs + '">' + mark + '<span class="cite__name">' + name + '</span>' + (plus ? '<span class="cite__plus">+' + plus + '</span>' : '') + '</span>';
}
function citeDoc(name: string, refs: string, plus?: number): string {
  return '<span class="cite cite--web cite--doc" data-refs="' + refs + '">' + ico('document-20-regular') + '<span class="cite__name">' + name + '</span>' + (plus ? '<span class="cite__plus">+' + plus + '</span>' : '') + '</span>';
}
function responseBlocksHtml(): string {
  ...
  return [
    '<button class="cotbar" id="cotBar" aria-expanded="false"><span class="cotbar__mark">' + ico('copilot-20-regular') + '</span><span class="cotbar__label">Reasoning completed in 8 steps</span><span class="cotbar__chev">' + ico('chevron-right-16-regular') + '</span></button>',
    '<p class="resp__p">I reviewed <a class="resp__link" data-refs="0" onclick="return false" href="#">John_Portfolio_FY26</a>, which contains 8 holdings: AAPL, MSFT, NVDA, AMZN, GOOGL, META, TSLA, and JPM, with share counts and average costs. Current market data shows most holdings are in large-cap AI/cloud/platform names, with <strong>TSLA</strong> the clear underperformer in 2026 YTD among the Mag 7 and <strong>JPM</strong> trading close to analyst median target, implying limited near-term upside. ' + citeWeb('LSEG', '1,0', 1) + '</p>',
    '<h4 class="resp__h4">Total valuation</h4>',
    '<div class="resp__summary">'
      + '<div class="resp__stat"><span class="resp__statl">Total market value</span><span class="resp__statv">$155,681</span></div>'
      + '<div class="resp__stat"><span class="resp__statl">Total cost basis</span><span class="resp__statv">$102,670</span></div>'
      + '<div class="resp__stat"><span class="resp__statl">Unrealized gain/loss</span><span class="resp__statv resp__pos">+$53,011 (+51.6%)</span></div>'
      + '</div>',
    '<h4 class="resp__h4">Portfolio snapshot</h4>',
    '<div class="resp__tablewrap"><table class="resp__table"><colgroup><col style="width:12%"/><col style="width:11%"/><col style="width:13%"/><col style="width:15%"/><col style="width:19%"/><col style="width:30%"/></colgroup><thead><tr>'
      + '<th>Ticker</th><th>Shares</th><th>Avg cost</th><th>Recent price</th><th>Approx. gain/loss</th><th>View</th>'
      + '</tr></thead><tbody>'
      + '<tr><td>AAPL</td><td>100</td><td>$185</td><td>$308.26</td><td class="resp__gl">+66.6%</td><td>Hold</td></tr>'
      + '<tr><td>MSFT</td><td>50</td><td>$420</td><td>$506.06</td><td class="resp__gl">+20.5%</td><td>Hold</td></tr>'
      + '<tr><td>NVDA</td><td>20</td><td>$130</td><td>$217.55</td><td class="resp__gl">+67.3%</td><td>Hold / trim if overweight</td></tr>'
      + '<tr><td>AMZN</td><td>60</td><td>$178</td><td>$278.09</td><td class="resp__gl">+56.2%</td><td>Hold</td></tr>'
      + '<tr><td>GOOGL</td><td>80</td><td>$164</td><td>$357.52</td><td class="resp__gl">+118.0%</td><td>Hold</td></tr>'
      + '<tr><td>META</td><td>40</td><td>$512</td><td>$594.92</td><td class="resp__gl">+16.2%</td><td>Hold / monitor</td></tr>'
      + '<tr><td>TSLA</td><td>30</td><td>$246</td><td>$330.88</td><td class="resp__gl">+34.5%</td><td>Sell / reduce</td></tr>'
      + '<tr><td>JPM</td><td>45</td><td>$198</td><td>$359.79</td><td class="resp__gl">+81.7%</td><td>Hold / trim</td></tr>'
      + '</tbody></table></div>',
    '<p class="resp__p">Prices and fundamentals for all holdings are from LSEG real-time market data as of market close Aug 10, 2026; analyst targets and consensus ratings are from LSEG. ' + citeWeb('LSEG Workspace', '1', 0) + '</p>',
    '<h4 class="resp__h4">Recommended actions</h4>',
    '<h5 class="resp__h5">Strong holds</h5>',
    '<div class="resp__rec"><div class="resp__rech">GOOGL \u2014 Hold</div><p class="resp__recp">GOOGL has the strongest unrealized gain in the portfolio, and current market commentary positions Alphabet as one of the more attractive Mag 7 names, with strong fundamentals, AI-enhanced search, Cloud improvement, Waymo optionality, and comparatively lower valuation versus peers. ' + citeWeb('LSEG', '3', 0) + '</p></div>',
    '<div class="resp__rec"><div class="resp__rech">AMZN \u2014 Hold</div><p class="resp__recp">AMZN is among the best 2026 YTD performers in the book, up +20.5% YTD per LSEG data, with AWS, advertising, and margin expansion as key drivers. ' + citeWeb('LSEG', '1,3', 1) + '</p></div>',
    '<div class="resp__rec"><div class="resp__rech">MSFT \u2014 Hold</div><p class="resp__recp">MSFT remains a high-quality enterprise AI/cloud holding. Current commentary highlights Azure, enterprise AI workloads, and Copilot monetization as key drivers, though the valuation premium should be watched. ' + citeWeb('LSEG', '3', 0) + '</p></div>',
    '<div class="resp__rec"><div class="resp__rech">AAPL \u2014 Hold</div><p class="resp__recp">AAPL is meaningfully above cost, and analyst consensus data from LSEG shows a Buy rating with median target of $330 versus a recent price of $308.26, implying moderate upside. I would hold rather than add aggressively because valuation is already elevated, with AAPL trading at a P/E around 35.5 in the LSEG data. ' + citeWeb('LSEG StarMine', '2', 0) + '</p></div>',
    '<h5 class="resp__h5">Hold, but monitor / consider trimming</h5>',
    '<div class="resp__rec"><div class="resp__rech">NVDA \u2014 Hold, trim only if position is too concentrated</div><p class="resp__recp">NVDA has strong gains and remains central to the AI infrastructure theme. Current commentary describes NVIDIA\u2019s AI hardware dominance and CUDA software moat, but also notes high valuation expectations. Given the holding is only 20 shares, I would hold, unless the client\u2019s overall portfolio is too concentrated in AI/semiconductor exposure. ' + citeWeb('LSEG', '5,3', 1) + '</p></div>',
    '<div class="resp__rec"><div class="resp__rech">META \u2014 Hold / monitor</div><p class="resp__recp">META is profitable and efficient, but compared with AMZN, GOOGL, AAPL, and NVDA, LSEG data shows META down around -9.9% YTD and -21.9% over 1 year. I would not sell immediately because core advertising and AI efficiency remain positives, but I\u2019d monitor underperformance and Reality Labs/regulatory risk. ' + citeWeb('LSEG', '1,3', 1) + '</p></div>',
    '<div class="resp__rec"><div class="resp__rech">JPM \u2014 Hold / trim</div><p class="resp__recp">JPM has a large unrealized gain from cost. LSEG data shows price at $359.79, median analyst target $371, and a Buy consensus from 26 analysts, implying only limited upside to the median target. I\u2019d hold for diversification and dividends, but consider trimming if the client wants to rebalance gains into underweight sectors or cash. ' + citeWeb('LSEG StarMine', '2', 0) + '</p></div>',
    '<h5 class="resp__h5">Sell / reduce</h5>',
    '<div class="resp__rec"><div class="resp__rech">TSLA \u2014 Sell or reduce</div><p class="resp__recp">TSLA is still above cost, but it is the weakest current market-trend fit in this portfolio. LSEG data shows TSLA as the weakest 2026 performer at -26.4% YTD, with weaker profitability versus peers, valuation dependency on future non-auto success, EV competition, and margin pressure. I would reduce or sell, especially if the client prefers quality/growth with stronger earnings visibility. ' + citeWeb('LSEG', '4,3', 1) + '</p></div>',
    '<h4 class="resp__h4">Bottom line</h4>',
    '<ul class="resp__ul">'
      + '<li><strong>Hold:</strong> AAPL, MSFT, NVDA, AMZN, GOOGL, META</li>'
      + '<li><strong>Hold / trim:</strong> JPM</li>'
      + '<li><strong>Sell / reduce:</strong> TSLA</li>'
      + '</ul>',
    '<p class="resp__p">If I had to prioritize: <strong>keep the core AI/cloud/platform winners \u2014 GOOGL, AMZN, MSFT, NVDA \u2014 and reduce TSLA first.</strong> For risk management, also consider trimming a portion of JPM after its strong run, since upside to analyst median target appears modest.</p>',
    '<div class="rfoot">',
    ...
    '<button class="rfoot__refs" id="rfRefs" data-refs="all" aria-label="References"><span class="rfoot__avatars">'
      + '<span class="rfoot__avatar rfoot__avatar--excel">' + brandSvg('excel-app-20-color') + '</span>'
      + '<span class="rfoot__avatar rfoot__avatar--lseg">' + LSEG_LOGO_IMG + '</span>'
      + avatar(ico('document-20-regular'), 'rfoot__avatar--doc')
      + '</span><span class="rfoot__refs-label">Sources</span></button>',
    '</div>',
    '<div class="resp__followups">'
      + '<button class="resp__chip resp__chip--icon"><img class="resp__chipico" src="' + logoImg('moodys-logo.png') + '" alt=""/>Go deeper with Moody\u2019s</button>'
      + '<button class="resp__chip">Suggest portfolio rebalancing strategies</button>'
      + '<button class="resp__chip">Highlight recent market news impacting these stocks</button>'
      + '</div>',
  ].join('');
}
...
function connectBanner(name: string, logoFile: string): string {
  const dismissIcon = '<svg viewBox="0 0 20 20" width="20" height="20" fill="none"><path d="M5 5l10 10M15 5 5 15" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"/></svg>';
  return '<div class="rbanner">'
    + '<span class="rbanner__logo"><img src="' + logoImg(logoFile) + '" alt=""/></span>'
    + '<span class="rbanner__main">'
      + '<span class="rbanner__title">Connect ' + name + ' data to Copilot</span>'
      + '<span class="rbanner__sub">Get insights from ' + name + ' to improve this response</span>'
    + '</span>'
    + '<button class="rbanner__connect" id="rbannerConnect">Connect</button>'
    + '<button class="rbanner__x" aria-label="Dismiss">' + dismissIcon + '</button>'
    + '</div>';
}

function discWeb(name: string): string {
  return '<span class="wcite"><span class="wcite__ico">' + ico('globe-20-regular') + '</span><span class="wcite__name">' + name + '</span></span>';
}

function discFile(title: string): string {
  return '<span class="wcite wcite--excel"><span class="wcite__ico">' + brandSvg('excel-app-20-color') + '</span><span class="wcite__name">' + title + '</span></span>';
}

function responseBlocksHtmlDisc(): string {
  const row = (ticker: string, shares: string, averageCost: string, recentPrice: string, view: string) =>
    '<tr><td>' + ticker + '</td><td>' + shares + '</td><td>' + averageCost + '</td><td>' + recentPrice + '</td><td>' + view + '</td></tr>';
  return [
    '<button class="cotbar" id="cotBar" aria-expanded="false"><span class="cotbar__mark">' + ico('copilot-20-regular') + '</span><span class="cotbar__label">Reasoning completed in 4 steps</span><span class="cotbar__chev">' + ico('chevron-right-16-regular') + '</span></button>',
    '<p class="resp__p">I reviewed <a class="resp__link" onclick="return false" href="#">John_Portfolio_FY26</a> (your attached workbook) and pulled current market context from the web. The recent prices and views below are approximate, drawn from public web sources. ' + discFile('John Portfolio FY26') + '</p>',
    '<h4 class="resp__h4">Portfolio snapshot</h4>',
    '<div class="resp__tablewrap"><table class="resp__table"><colgroup><col style="width:14%"/><col style="width:12%"/><col style="width:15%"/><col style="width:22%"/><col style="width:37%"/></colgroup><thead><tr>'
      + '<th>Ticker</th><th>Shares</th><th>Avg cost</th><th>Recent price (approx.)</th><th>Leaning</th>'
      + '</tr></thead><tbody>'
      + row('AAPL', '100', '$185', '~$305', 'Hold')
      + row('MSFT', '50', '$420', '~$505', 'Hold')
      + row('NVDA', '20', '$130', '~$216', 'Hold')
      + row('AMZN', '60', '$178', '~$277', 'Hold')
      + row('GOOGL', '80', '$164', '~$356', 'Hold')
      + row('META', '40', '$512', '~$592', 'Hold / monitor')
      + row('TSLA', '30', '$246', '~$330', 'Reduce / watch')
      + row('JPM', '45', '$198', '~$358', 'Hold')
      + '</tbody></table></div>',
    '<p class="resp__p">Shares and average cost are from your workbook; recent prices are indicative, from public web sources today and may differ from your broker or live market data. ' + discFile('John Portfolio FY26') + ' ' + discWeb('finance.yahoo.com') + '</p>',
    '<h4 class="resp__h4">What public market coverage suggests</h4>',
    '<ul class="resp__ul">'
      + '<li><strong>Core holds:</strong> GOOGL, AMZN, MSFT, NVDA - public coverage broadly frames the AI / cloud leaders as the strongest of the group. ' + discWeb('Reuters') + '</li>'
      + '<li><strong>Monitor:</strong> META and JPM - solid, but commentary is more mixed on near-term upside. ' + discWeb('MarketWatch') + '</li>'
      + '<li><strong>Weakest:</strong> TSLA - widely described as the softest 2026 performer among the Mag 7. ' + discWeb('CNBC') + '</li>'
      + '</ul>',
    '<h4 class="resp__h4">Bottom line</h4>',
    '<p class="resp__p">From public web coverage, keep the AI / cloud winners (GOOGL, AMZN, MSFT, NVDA) as core holds and treat TSLA as the one to trim first. These are approximate, based on public market coverage rather than precise valuations or analyst consensus. ' + discWeb('finance.yahoo.com') + '</p>',
    '<div class="rfoot"><div class="rfoot__toolbar">',
    '<button class="rfoot__btn" title="Copy" aria-label="Copy">' + ico('copy-20-regular') + '</button>',
    '<button class="rfoot__btn rfoot__btn--toggle" id="rfLike" title="Good response" aria-label="Good response"><span class="ico-r">' + ico('thumb-like-20-regular') + '</span><span class="ico-f">' + ico('thumb-like-20-filled') + '</span></button>',
    '<button class="rfoot__btn rfoot__btn--toggle" id="rfDislike" title="Bad response" aria-label="Bad response"><span class="ico-r">' + ico('thumb-dislike-20-regular') + '</span><span class="ico-f">' + ico('thumb-dislike-20-filled') + '</span></button>',
    '<button class="rfoot__btn" title="Try again" aria-label="Try again">' + ico('arrow-sync-20-regular') + '</button>',
    '<button class="rfoot__btn" title="More actions" aria-label="More actions">' + ico('more-horizontal-20-regular') + '</button>',
    '</div><div class="rfoot__divider"></div>',
    '<span class="rfoot__refs"><span class="rfoot__avatars">'
      + '<span class="rfoot__avatar rfoot__avatar--excel">' + brandSvg('excel-app-20-color') + '</span>'
      + '<span class="rfoot__avatar">' + ico('globe-20-regular') + '</span>'
      + '</span><span class="rfoot__refs-label">Sources</span></span>',
    '</div>',
    connectBanner('LSEG', 'lseg-logo.png'),
  ].join('');
}
...
interface RefSrc { fav?: string; letter?: string; source: string; headline: string; url: string; file?: boolean; lseg?: boolean; excel?: boolean; }
const RESP_REFS: RefSrc[] = [
  { source: 'John_Portfolio_FY26.xlsx', headline: 'Client holdings \u2014 8 positions (shares & avg cost)', url: 'Excel \u00b7 attached workbook', file: true, excel: true },
  { source: 'LSEG Workspace', headline: 'Real-time prices & quotes \u2014 AAPL, MSFT, NVDA, AMZN, GOOGL, META, TSLA, JPM', url: 'workspace.lseg.com/equities/quotes', lseg: true },
  { source: 'LSEG StarMine', headline: 'Analyst consensus, target prices & Buy / Hold / Sell ratings', url: 'lseg.com/data-analytics/starmine/consensus', lseg: true },
  { source: 'LSEG Equity Research', headline: 'Mag 7 positioning \u2014 hold / sell strategy & sector rotation', url: 'lseg.com/insights/mag-7-hold-sell-strategy', lseg: true },
  { source: 'LSEG \u2014 TSLA', headline: 'Tesla fundamentals & 2026 YTD performance (\u221226.4%)', url: 'lseg.com/data-analytics/equities/tsla', lseg: true },
  { source: 'LSEG \u2014 NVDA', headline: 'NVIDIA / AI-semiconductor coverage & valuation', url: 'lseg.com/data-analytics/equities/nvda', lseg: true },
  { source: 'Contoso Wealth \u2014 Suitability & Advice Policy', headline: 'Firm guidance on suitability, disclosures & advice', url: 'SharePoint \u00b7 internal documentation', file: true },
];
function cpRow(r: RefSrc): string {
  const fav = r.excel
    ? '<span class="cp__fav cp__fav--excel">' + brandSvg('excel-app-20-color') + '</span>'
    : (r.lseg
    ? '<span class="cp__fav cp__fav--lseg">' + LSEG_LOGO_IMG + '</span>'
    : (r.file
...
function refsPopover(): string {
  return '<div class="cp" id="refsPop" role="dialog" aria-label="References"></div>';
}

// ─── CSS ────────────────────────────────────────────────────

const css = `
@font-face { font-family: 'Segoe Sans Skills'; src: url('../fonts/Segoe-sans.woff2') format('woff2-variations'), url('../fonts/Segoe-sans.woff2') format('woff2'); font-weight: 100 900; font-style: normal; font-display: swap; }
...
.nav--collapsed .nav__logo { display: none; }
...
/* ─── Conversation mode ─── */
.thread { display: none; }
.shell--convo .body { align-items: stretch; padding: 0; min-height: 0; }
...
.shell--convo .thread { display: flex; flex-direction: column; gap: 24px; flex: 1 1 auto; min-height: 0; width: 100%; overflow-y: auto; scrollbar-width: none; -ms-overflow-style: none; padding: 24px 4px 8px; }
.shell--convo .thread::-webkit-scrollbar { width: 0; height: 0; display: none; }
...
/* User message bubble */
.umsg { display: flex; flex-direction: column; align-items: flex-end; }
.umsg__files { display: flex; justify-content: flex-end; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; }
.umsg__file { display: inline-flex; align-items: center; gap: 8px; height: 40px; padding: 0 14px 0 10px; border-radius: 10px; background: #f0f0f0; }
.umsg__file svg, .umsg__file img { width: 20px; height: 20px; object-fit: contain; }
.umsg__filename { font-size: 14px; line-height: 20px; font-weight: 420; color: #242424; max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
/* Copilot response */
.resp { display: flex; flex-direction: column; gap: 16px; max-width: 720px; }
.resp--reveal > * { opacity: 0; transform: translateY(8px); }
.resp--reveal > *.is-shown { opacity: 1; transform: none; transition: opacity 0.4s ease, transform 0.4s ease; }
@media (prefers-reduced-motion: reduce) { .resp--reveal > * { opacity: 1; transform: none; transition: none; } }
...
.resp__h5 { font-size: 18px; line-height: 24px; font-weight: 600; font-variation-settings: 'opsz' 12, 'wght' 625; color: #242424; margin-top: 8px; }
.resp__ul { margin: 0 0 0 22px; padding: 0; display: flex; flex-direction: column; gap: 6px; }
.resp__ul li { font-size: 16px; line-height: 26px; font-weight: 420; color: #242424; }
.resp__ul li strong { font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; }
.resp__pos { color: #0e700e; }
.resp__neg { color: #b10e1c; }
/* Response data table — bordered, rounded, header row (matches attachment/Excel table style) */
.resp__tablewrap { margin: 6px 0; overflow-x: auto; }
.resp__table { width: 100%; border-collapse: separate; border-spacing: 0; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; font-size: 14px; line-height: 20px; }
.resp__table th { text-align: left; font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; color: #242424; padding: 12px 16px; background: #fff; border-bottom: 1px solid #e0e0e0; white-space: nowrap; }
.resp__table td { padding: 12px 16px; color: #242424; border-bottom: 1px solid #ededed; white-space: nowrap; }
.resp__table tr:last-child td { border-bottom: none; }
.resp__table tr.resp__trtotal td { font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; border-top: 1px solid #e0e0e0; }
.resp__pill { display: inline-flex; align-items: center; height: 22px; padding: 0 10px; border-radius: 9999px; font-size: 12px; line-height: 16px; font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; }
.resp__pill--hold { background: #e7f2ff; color: #0f548c; }
.resp__pill--add { background: #e5f4e9; color: #0e700e; }
.resp__pill--trim { background: #fdf3e2; color: #8a5a00; }
.resp__pill--sell { background: #fdeaea; color: #b10e1c; }
.resp__summary { display: flex; gap: 12px; margin: 8px 0 4px; flex-wrap: wrap; }
.resp__stat { flex: 1 1 0; min-width: 150px; border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 4px; }
.resp__statl { font-size: 12px; line-height: 16px; color: #5d5d5d; }
.resp__statv { font-size: 20px; line-height: 26px; font-weight: 600; font-variation-settings: 'opsz' 12, 'wght' 625; color: #242424; }
.resp__table td.resp__gl { font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; }
.resp__rec { margin-top: 10px; }
.resp__rech { font-size: 16px; line-height: 22px; font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 625; color: #242424; margin-bottom: 2px; }
.resp__recp { font-size: 16px; line-height: 26px; font-weight: 420; color: #242424; }
.resp__followups { display: flex; flex-direction: column; align-items: flex-end; gap: 8px; margin-top: 14px; }
.resp__chip { border: 1px solid #dedede; border-radius: 9999px; background: #fff; font-family: inherit; padding: 8px 14px; font-size: 14px; line-height: 18px; color: #242424; cursor: pointer; transition: background 0.1s; }
.resp__chip--icon { display: inline-flex; align-items: center; gap: 8px; padding-left: 10px; }
.resp__chipico { width: 18px; height: 18px; border-radius: 4px; object-fit: contain; flex-shrink: 0; }
.resp__chip:hover { background: rgba(36,36,36,0.04); }
.wcite { display: inline-flex; align-items: center; gap: 4px; height: 20px; padding: 0 8px 0 5px; margin: 0 1px; border-radius: 9999px; background: #f5f5f5; vertical-align: -4px; }
.wcite__ico { display: inline-flex; width: 14px; height: 14px; flex-shrink: 0; color: #5d5d5d; }
.wcite__ico svg, .wcite__ico img { width: 14px; height: 14px; display: block; }
.wcite--excel .wcite__ico { border-radius: 3px; overflow: hidden; }
.wcite__name { font-size: 12px; line-height: 16px; color: #5d5d5d; white-space: nowrap; }
.rbanner { display: flex; align-items: center; gap: 12px; padding: 16px; margin-top: 18px; margin-bottom: 88px; border: 1px solid #dedede; border-radius: 16px; background: #fff; }
.rbanner__logo { width: 40px; height: 40px; border-radius: 4px; overflow: hidden; flex-shrink: 0; }
.rbanner__logo img { width: 100%; height: 100%; object-fit: cover; display: block; }
.rbanner__main { flex: 1; min-width: 0; display: flex; flex-direction: column; }
.rbanner__title { font-size: 14px; line-height: 20px; font-weight: 625; font-variation-settings: 'opsz' 8, 'wght' 625; color: #242424; }
.rbanner__sub { font-size: 14px; line-height: 20px; color: #5d5d5d; }
.rbanner__connect { height: 32px; padding: 6px 12px; border: none; border-radius: 12px; background: #f2f2f2; color: #242424; font-family: inherit; font-size: 14px; font-weight: 625; font-variation-settings: 'opsz' 8, 'wght' 625; cursor: pointer; flex-shrink: 0; transition: background 0.1s; }
.rbanner__connect:hover { background: #ebebeb; }
.rbanner__connected { display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px 4px 6px 0; border-radius: 8px; color: #424242; font-size: 14px; line-height: 20px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; flex-shrink: 0; }
.rbanner__check { width: 20px; height: 20px; display: inline-flex; color: #017048; flex-shrink: 0; }
.rbanner__check svg { width: 20px; height: 20px; display: block; }
.rbanner__x { width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center; border: none; border-radius: 8px; background: transparent; color: #616161; cursor: pointer; flex-shrink: 0; transition: background 0.1s; }
.rbanner__x:hover { background: rgba(0,0,0,0.04); }
.rbanner__x svg { width: 20px; height: 20px; }
...
.cite--file { padding: 3px 5px; }
.cite--doc svg { color: #5d5d5d; }
...
/* Reasoning thinking animation — static header + spinning Copilot mark on the active step */
.rthink { display: flex; flex-direction: column; gap: 10px; }
.rthink__head { display: inline-flex; align-items: center; gap: 8px; font-size: 14px; line-height: 20px; color: #424242; }
.rthink__ff { display: inline-flex; width: 16px; height: 16px; color: #616161; }
.rthink__ff svg { width: 16px; height: 16px; }
.rthink__step { display: inline-flex; align-items: center; gap: 8px; }
.rthink__mark { display: inline-flex; align-items: center; justify-content: center; width: 26px; height: 26px; flex-shrink: 0; }
.rthink__mark img { width: 26px; height: 26px; object-fit: contain; display: block; }
.rthink__label { font-size: 16px; line-height: 28px; color: #242424; background: linear-gradient(100deg, #242424 30%, #bdbdbd 50%, #242424 70%); background-size: 200% 100%; -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; animation: thinkShimmer 1.6s linear infinite; }
@media (prefers-reduced-motion: reduce) { .rthink__label { animation: none; -webkit-text-fill-color: #242424; } }
.rthink__chev { display: inline-flex; width: 16px; height: 16px; color: #6f6f6f; }
.rthink__chev svg { width: 16px; height: 16px; }

/* Reasoning chip (toggles the CoT panel) */
.cotbar { display: inline-flex; align-items: center; gap: 8px; align-self: flex-start; height: 32px; padding: 0 8px; border: none; border-radius: 8px; background: transparent; color: #616161; cursor: pointer; transition: background 0.1s; }
.cotbar:hover { background: rgba(36,36,36,0.06); }
...
.rfoot__avatar--l { background: #001eff; }
.rfoot__avatar--l2 { background: #0f6cbd; }
.rfoot__avatar--lseg { background: transparent; border-radius: 5px; overflow: hidden; }
.rfoot__avatar--lseg svg { width: 20px; height: 20px; }
.rfoot__avatar--lseg img { width: 20px; height: 20px; object-fit: contain; }
.rfoot__avatar--excel { background: #fff; }
.rfoot__avatar--excel svg { width: 14px; height: 14px; }
.rfoot__avatar--doc { background: #dbe6f5; color: #0f6cbd; }
...
.cp__fav--excel { background: transparent; }
.cp__fav--excel svg { width: 16px; height: 16px; }
.cp__fav--lseg { background: transparent; overflow: hidden; }
.cp__fav--lseg svg { width: 16px; height: 16px; }
.cp__fav--lseg img { width: 16px; height: 16px; object-fit: contain; border-radius: 3px; }
...
.cot-list { position: relative; }
.cot-list::before { content: ''; position: absolute; left: 12px; top: 20px; bottom: 20px; width: 1px; background: #e2e2e2; }
.cot-item { position: relative; }
.cot-step { display: flex; align-items: center; gap: 8px; width: 100%; padding: 9px 4px; border: none; background: transparent; cursor: pointer; text-align: left; }
.cot-step__chev { display: inline-flex; width: 16px; height: 16px; color: #616161; flex-shrink: 0; position: relative; z-index: 1; background: #fff; transition: transform 0.15s ease; }
.cot-step__chev svg { width: 16px; height: 16px; }
.cot-item.is-open .cot-step__chev { transform: rotate(90deg); }
.cot-step__label { font-size: 14px; line-height: 20px; color: #242424; }
.cot-step__body { display: none; padding: 0 0 10px 24px; }
.cot-item.is-open .cot-step__body { display: block; }
.cot-step__body p { font-size: 13px; line-height: 18px; color: #616161; margin: 0 0 6px; }
.cot-subs { list-style: none; margin: 0; padding: 0; }
.cot-subs li { position: relative; font-size: 13px; line-height: 18px; color: #616161; padding: 3px 0 3px 16px; }
.cot-subs li::before { content: ''; position: absolute; left: 2px; top: 10px; width: 4px; height: 4px; border-radius: 9999px; background: #bdbdbd; }
...
.cmp__row { display: flex; align-items: center; gap: 4px; position: relative; }
.cmp__ta { display: block; width: 100%; box-sizing: border-box; min-width: 0; border: none; outline: none; resize: none; background: transparent; overflow: hidden; font-family: 'Segoe Sans','Segoe UI',system-ui,sans-serif; font-size: 16px; line-height: 28px; color: #1f1f1f; padding: 0 8px; margin-bottom: 8px; caret-color: #242424; field-sizing: content; }
.cmp__ta::placeholder { color: #6f6f6f; }
.cmp__ghost { position: absolute; display: none; pointer-events: none; color: #6f6f6f; white-space: pre; font-family: 'Segoe Sans','Segoe UI',system-ui,sans-serif; font-size: 16px; line-height: 28px; }
...
.cmp.ci--has-text ~ .zs__chips { display: none; }
...
.stgd__id { position: sticky; top: 0; z-index: 5; display: flex; align-items: center; gap: 14px; padding: 25px 0 18px; background: #fff; }
.stgd__logo { width: 28px; height: 28px; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; border-radius: 5.657px; overflow: hidden; }
.stgd__logo img { width: 28px; height: 28px; object-fit: contain; display: block; }
.stgd__logo .stg__gi { width: 28px; height: 28px; }
.stgd__logo svg { width: 28px; height: 28px; }
.stgd__namewrap { flex: 1; min-width: 0; display: flex; align-items: center; gap: 8px; }
.stgd__name { min-width: 0; font-size: 16px; line-height: 28px; font-weight: 625; font-variation-settings: 'opsz' 12, 'wght' 625; color: #242424; }
...
.stgd__admin { display: inline-flex; align-items: center; height: 32px; padding: 0 10px; font-size: 14px; line-height: 20px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; color: #929292; white-space: nowrap; }
...
.stgd__rule { position: sticky; top: 75px; z-index: 5; height: 1px; background: #dedede; margin: 0; border: none; }
.stgd__intro { margin: 0; padding: 16px 0; }
.stgd__intro::before { content: 'About'; display: block; margin-bottom: 4px; color: #242424; font-size: 14px; line-height: 20px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; }
.stgd__sec { padding: 16px 0; }
.stgd__sech { font-size: 14px; line-height: 20px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; color: #242424; }
.stgd__sech + .stgd__chips, .stgd__sech + .stgd__mcpcard { margin-top: 8px; }
.stgd__p { font-size: 14px; line-height: 20px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; color: #6f6f6f; }
.stgd__chips { display: flex; flex-wrap: wrap; align-items: center; gap: 8px; }
.stgd__chip { display: inline-flex; align-items: center; justify-content: center; min-height: 24px; padding: 2px 10px; border: none; border-radius: 9999px; background: rgba(0,0,0,0.05); color: #242424; font-family: 'Segoe Sans Skills','Segoe Sans','Segoe UI',sans-serif; font-size: 12px; line-height: 16px; font-weight: 400; font-variation-settings: 'opsz' 8, 'wght' 400; letter-spacing: 0; white-space: nowrap; }
.stgd__chip[hidden] { display: none; }
.stgd__skills-toggle { display: inline-flex; align-items: center; justify-content: center; gap: 2px; height: 24px; padding: 4px 8px; border: none; border-radius: 8px; background: transparent; color: #242424; font-family: 'Segoe Sans Skills','Segoe Sans','Segoe UI',sans-serif; font-size: 12px; line-height: 16px; font-weight: 420; font-variation-settings: 'opsz' 8, 'wght' 420; letter-spacing: 0; white-space: nowrap; cursor: pointer; transition: background 0.1s; }
.stgd__skills-toggle:hover { background: rgba(36,36,36,0.04); }
.stgd__skills-toggle svg { width: 16px; height: 16px; flex: 0 0 auto; transition: transform 0.12s; }
.stgd__skills-toggle.is-expanded svg { transform: rotate(180deg); }
.stgd__skills-toggle--lseg { width: 71px; }
.stgd__skills-toggle--lseg.is-expanded { width: 81px; }
...
.stgd__meta { display: flex; flex-direction: column; }
.stgd__row { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 8px 0; }
...
.stgd__row--more { align-items: flex-start; padding: 0; }
.stgd__row--more > .stgd__label { padding: 8px 0; }
.stgd__links2 { display: inline-flex; flex-direction: column; align-items: flex-end; }
.stgd__links2 .stgd__link { height: 36px; gap: 2px; font-size: 16px; line-height: 28px; }
...
.ad { width: 640px; max-width: calc(100vw - 48px); max-height: calc(100vh - 48px); overflow-y: auto; background: #fff; border-radius: 24px; padding: 32px 40px; position: relative; display: flex; flex-direction: column; gap: 24px;
  box-shadow: 0 0 2px 0 rgba(0,0,0,0.15), 0 8px 16px 0 rgba(0,0,0,0.03), 0 32px 48px 0 rgba(0,0,0,0.08); }
...
.ad__close--abs { position: absolute; top: 24px; right: 24px; }
.ad__hand { display: flex; align-items: center; justify-content: center; gap: 16px; }
.ad__cop { display: inline-flex; align-items: center; justify-content: center; width: 48px; height: 48px; color: #242424; flex-shrink: 0; }
.ad__cop svg { width: 48px; height: 48px; }
.ad__dots { display: inline-flex; align-items: center; justify-content: center; color: #616161; flex-shrink: 0; }
.ad__dots svg { width: 32px; height: 32px; }
.ad__logo--lg { width: 48px; height: 48px; }
.ad__logo--lg img { width: 48px; height: 48px; border-radius: 12px; object-fit: contain; }
.ad__logo--lg svg { width: 48px; height: 48px; }
...
.sso { position: fixed; inset: 0; z-index: 700; background: #edecea; display: none; align-items: center; justify-content: center; font-family: 'Segoe UI', Arial, sans-serif; }
.sso.is-open { display: flex; }
.sso__card { width: 360px; max-width: calc(100vw - 32px); background: #fff; border-radius: 8px; box-shadow: 0 2px 12px rgba(0,0,0,0.14); overflow: hidden; }
.sso__body { padding: 28px 32px 24px; display: flex; flex-direction: column; }
.sso__brand { display: flex; align-items: center; gap: 10px; margin-bottom: 20px; }
.sso__logo { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; flex-shrink: 0; }
.sso__logo img { width: 28px; height: 28px; object-fit: contain; border-radius: 6px; }
.sso__logo svg { width: 28px; height: 28px; }
.sso__name { font-size: 18px; font-weight: 600; color: #242424; }
.sso__title { font-size: 20px; font-weight: 600; color: #242424; margin-bottom: 20px; }
.sso__label { font-size: 13px; color: #616161; margin-bottom: 4px; }
.sso__field { display: flex; align-items: center; border: 1px solid #8a8a8a; border-radius: 4px; height: 40px; padding: 0 10px; margin-bottom: 12px; }
.sso__field:focus-within { border-color: #0f6cbd; box-shadow: 0 0 0 1px #0f6cbd; }
.sso__field input { flex: 1; min-width: 0; border: none; outline: none; background: none; font-family: inherit; font-size: 14px; color: #242424; }
.sso__link { align-self: flex-start; color: #0f6cbd; font-size: 13px; margin: 2px 0 20px; text-decoration: none; cursor: pointer; }
.sso__link:hover { text-decoration: underline; }
.sso__actions { display: flex; justify-content: flex-end; gap: 8px; }
.sso__cancel { background: transparent; border: 1px solid #d1d1d1; color: #242424; font-family: inherit; font-size: 14px; font-weight: 600; padding: 8px 18px; border-radius: 4px; cursor: pointer; }
.sso__cancel:hover { background: rgba(0,0,0,0.04); }
.sso__signin { background: #0f6cbd; border: 1px solid #0f6cbd; color: #fff; font-family: inherit; font-size: 14px; font-weight: 600; padding: 8px 18px; border-radius: 4px; cursor: pointer; }
.sso__signin:hover { background: #115ea3; }
.sso__foot { display: flex; align-items: center; gap: 6px; padding: 12px 32px; background: #f5f5f5; border-top: 1px solid #ebebeb; font-size: 12px; color: #616161; }
.sso__lock { display: inline-flex; flex-shrink: 0; }
...
.cmp__attach { display: none; align-items: center; flex-wrap: nowrap; gap: 8px; min-width: 0; }
.cmp__attach:not(:empty) { display: flex; }
.cmp__spacer { flex: 1 1 auto; min-width: 8px; }
...
.cmp-attach__x { position: absolute; top: 6px; right: 6px; display: inline-flex; align-items: center; justify-content: center; width: 16px; height: 16px; padding: 0; border: none; background: none; border-radius: 4px; color: #616161; cursor: pointer; opacity: 0; pointer-events: none; transition: opacity 0.1s, background 0.1s, color 0.1s; }
.cmp-attach:hover .cmp-attach__x, .cmp-attach:focus-within .cmp-attach__x { opacity: 1; pointer-events: auto; }
.cmp-attach__x:hover { background: rgba(36,36,36,0.06); color: #242424; }
.cmp-attach__x svg { width: 12px; height: 12px; }
.cmp-attach--skill { height: 40px; padding: 6px; padding-right: 32px; gap: 8px; border: none; border-radius: 9999px; background: #f5f5f5; }
.cmp-attach--skill .cmp-attach__ico { width: 24px; height: 24px; border-radius: 6px; background: transparent; }
.cmp-attach--skill .cmp-attach__ico svg { width: 20px; height: 20px; }
.cmp-attach--skill .cmp-attach__ico img { width: 24px; height: 24px; border-radius: 6px; }
.cmp-attach--skill .cmp-attach__name { max-width: 160px; font-weight: 600; font-variation-settings: 'opsz' 8, 'wght' 600; }
.cmp-attach--skill .cmp-attach__x { top: 50%; transform: translateY(-50%); right: 10px; opacity: 1; pointer-events: auto; }
...
.ciq__ico img, .ciq__ico svg { width: 20px; height: 20px; display: block; }
.ciq__ico img { object-fit: cover; border-radius: 5px; }
.ciq__ico svg { object-fit: contain; }
...
.stgd__skhead { display: flex; align-items: center; gap: 14px; padding: 25px 0 18px; border-bottom: 1px solid #dedede; }
...
.stgd__skhname { font-size: 16px; line-height: 28px; font-weight: 600; font-variation-settings: 'opsz' 12, 'wght' 600; color: #242424; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.stgd__skhtry { display: inline-flex; align-items: center; justify-content: center; height: 32px; padding: 6px 10px; border: none; border-radius: 12px; background: #242424; color: #fff; font-family: inherit; font-size: 14px; line-height: 20px; cursor: pointer; transition: background 0.1s; white-space: nowrap; }
.stgd__skhtry:hover { background: #2b2b2b; }
...
const js = [
  ...
  "  (function(){ var wrap0 = document.getElementById('cmpAttach'); if (wrap0){ wrap0.querySelectorAll('.cmp-attach__x').forEach(function(x){ x.addEventListener('click', function(e){ e.stopPropagation(); var pill = x.closest('.cmp-attach'); if (pill) pill.remove(); syncSend(); }); }); } syncSend(); })();",
  "  var CIQ_TABS = " + JSON.stringify(CIQ_TABS) + ";",
  "  var CIQ_MTABS = " + JSON.stringify(CIQ_MTABS) + ";",
  "  var CIQ_SKILL_EXCLUDE = " + JSON.stringify(CIQ_SKILL_EXCLUDE) + ";",
  ...
  "  function setSkillToken(tok){ if (!ta) return; var value = ta.value; var idx = value.lastIndexOf('/'); var trigger = idx !== -1 && (idx === 0 || value.charAt(idx - 1) === ' ') && value.slice(idx).indexOf(' ') === -1; if (trigger){ value = value.slice(0, idx); } if (value.length && value.charAt(value.length - 1) !== ' ') value += ' '; value += tok + ' '; ta.value = value; try { ta.setSelectionRange(value.length, value.length); } catch(e){} }",
  "  function tokenPresent(tok){ return ((' ' + ta.value + ' ').indexOf(' ' + tok + ' ') !== -1); }",
  "  function removeSkillToken(tok){ if (!ta) return; var seg = ' ' + tok + ' '; var padded = ' ' + ta.value + ' '; var at = padded.indexOf(seg); if (at === -1) return; padded = padded.slice(0, at) + ' ' + padded.slice(at + seg.length); ta.value = padded.slice(1, padded.length - 1); }",
  "  function reconcileSkillPills(){ var wrap = document.getElementById('cmpAttach'); if (!wrap || !ta) return; var pills = wrap.querySelectorAll('.cmp-attach--skill[data-token]'); for (var i = 0; i < pills.length; i++){ var tok = pills[i].getAttribute('data-token'); if (!tokenPresent(tok)) pills[i].remove(); } syncSend(); }",
  "  function ciqAttach(r){ var wrap = document.getElementById('cmpAttach'); var skillTok = r.kind === 'skill' ? (r.title.charAt(0) === '/' ? r.title : '/' + r.title) : null; if (wrap){ var key = r.title; if (!wrap.querySelector('[data-ciq=\"' + key + '\"]')){ var el = document.createElement('div'); el.setAttribute('data-ciq', key); el.setAttribute('title', r.title); if (r.kind === 'skill'){ el.className = 'cmp-attach cmp-attach--skill'; el.setAttribute('data-token', skillTok); el.innerHTML = '<span class=\"cmp-attach__ico\">' + r.icon + '</span><span class=\"cmp-attach__name\">' + skillTok + '</span><button class=\"cmp-attach__x\" aria-label=\"Remove ' + skillTok + '\">' + X12_SVG + '</button>'; } else { el.className = 'cmp-attach'; el.innerHTML = '<span class=\"cmp-attach__ico\">' + r.icon + '</span><span class=\"cmp-attach__meta\"><span class=\"cmp-attach__name\">' + r.title + '</span><span class=\"cmp-attach__desc\">' + r.sub + '</span></span><button class=\"cmp-attach__x\" aria-label=\"Remove\">' + X12_SVG + '</button>'; } el.querySelector('.cmp-attach__x').addEventListener('click', function(e){ e.stopPropagation(); if (skillTok) removeSkillToken(skillTok); el.remove(); syncSend(); }); wrap.appendChild(el); } } if (skillTok) setSkillToken(skillTok); closeCiq(); closeCiqModal(); syncSend(); if (ta) ta.focus(); }",
  ...
  "  function ciqSkillRows(){ var out = []; if (typeof SRC === 'undefined' || !SRC) return out; var sources = SRC.filter(function(s){ return !s.sync && CIQ_SKILL_EXCLUDE.indexOf(s.n) === -1; }).sort(function(a, b){ var ar = a.rec ? 0 : 1, br = b.rec ? 0 : 1; if (ar !== br) return ar - br; var an = a.n.toLowerCase(), bn = b.n.toLowerCase(); return an < bn ? -1 : (an > bn ? 1 : 0); }); sources.forEach(function(s){ var sk = (typeof SRC_SKILLS !== 'undefined' && SRC_SKILLS[s.n]) || []; sk.forEach(function(k){ out.push({ icon: s.stgLogo, title: '/' + k.name, sub: k.desc, kind: 'skill' }); }); }); return out; }",
  ...
  "  function openCiq(id, q){ if (!ciqMenu) return; ciqRender(CIQ_TABS, id || 'all', ciqTabs, ciqList, q); ciqMenu.hidden = false; if (zsEl) zsEl.classList.add('ciq-open'); }",
  "  function closeCiq(){ if (!ciqMenu) return; ciqMenu.hidden = true; ciqSlash = false; var _g = document.getElementById('cmpGhost'); if (_g) _g.style.display = 'none'; if (zsEl) zsEl.classList.remove('ciq-open'); }",
  ...
  "  if (ciqTabs) ciqTabs.addEventListener('click', function(e){ var t = e.target.closest('.ciq__tab'); if (t){ e.stopPropagation(); ciqRender(CIQ_TABS, t.getAttribute('data-tab'), ciqTabs, ciqList); } });",
  "  if (ciqmTabs) ciqmTabs.addEventListener('click', function(e){ var t = e.target.closest('.ciq__tab'); if (t){ e.stopPropagation(); ciqmTab = t.getAttribute('data-tab'); ciqmPaint(); } });",
  ...
  "  function ciqGhostEl(){ var g = document.getElementById('cmpGhost'); if (!g && ta){ g = document.createElement('span'); g.id = 'cmpGhost'; g.className = 'cmp__ghost'; g.setAttribute('aria-hidden', 'true'); g.textContent = 'Type to search'; ta.parentNode.appendChild(g); } return g; }",
  "  function updateCiqGhost(show){ var g = ciqGhostEl(); if (!g || !ta) return; if (!show){ g.style.display = 'none'; return; } var m = ta.__ghostMirror; if (!m){ m = document.createElement('div'); m.setAttribute('aria-hidden', 'true'); m.style.cssText = 'position:absolute;visibility:hidden;top:0;left:-9999px;white-space:pre-wrap;word-wrap:break-word;'; ta.parentNode.appendChild(m); ta.__ghostMirror = m; } var cs = getComputedStyle(ta); ['fontFamily','fontSize','fontWeight','fontVariationSettings','letterSpacing','lineHeight','paddingTop','paddingRight','paddingBottom','paddingLeft','borderTopWidth','borderRightWidth','borderBottomWidth','borderLeftWidth','boxSizing'].forEach(function(p){ m.style[p] = cs[p]; }); m.style.width = ta.clientWidth + 'px'; m.textContent = ta.value; var marker = document.createElement('span'); marker.textContent = '\\u200b'; m.appendChild(marker); g.style.display = 'block'; g.style.left = (ta.offsetLeft + marker.offsetLeft - ta.scrollLeft) + 'px'; g.style.top = (ta.offsetTop + (ta.clientHeight - (parseFloat(cs.lineHeight) || 28)) / 2 - ta.scrollTop) + 'px'; m.removeChild(marker); }",
  "  if (ta) ta.addEventListener('input', function(){ var v = ta.value; var idx = v.lastIndexOf('/'); var ok = idx !== -1 && (idx === 0 || v.charAt(idx-1) === ' ') && v.slice(idx).indexOf(' ') === -1; if (ok){ var q = v.slice(idx + 1); openCiq('skills', q); updateCiqGhost(q.length === 0); } else { closeCiq(); updateCiqGhost(false); } reconcileSkillPills(); });",
  ...
  "  var REASON_STEPS = ['Reading attached file', 'Searching market data', 'Searching for finance skills', 'Loading market researcher skill', 'Connecting to LSEG', 'Pulling live quotes and fundamentals', 'Assessing market trends', 'Generating recommendations'];",
  "  var RESP_HTML_CONNECTED = RESP_HTML; var REASON_STEPS_CONNECTED = REASON_STEPS.slice(); var _initialCotList = document.querySelector('#spPanel .cot-list'); var COT_CONNECTED_STEPS = _initialCotList ? _initialCotList.innerHTML : ''; var LSEG_TRY_PROMPT = 'Answer with London Stock Exchange Group data';",
  "  var RESP_HTML_DISC = " + JSON.stringify('<div class="resp" id="respBlk">' + responseBlocksHtmlDisc() + '</div>') + ";",
  "  var REASON_STEPS_DISC = ['Reading attached file', 'Searching the web for market context', 'Assessing market trends', 'Generating recommendations'];",
  "  var COT_DISC_STEPS = " + JSON.stringify(cotStep('Reading attached file', 'Scanning the workbook to understand the holdings.', ['Opened John Portfolio FY26.xlsx and read the holdings tab', 'Identified 8 positions and average costs']) + cotStep('Searching the web for market context', 'Gathering public market context to frame the analysis.', ['Searched the web for 2026 market outlook and Mag 7 performance', 'Checked 2026 YTD performance and recent public commentary', 'Used public sources for approximate recent prices']) + cotStep('Assessing market trends', 'Weighing signals from public coverage.', ['Weighed momentum and public commentary per holding', 'Flagged that figures are approximate']) + cotStep('Generating recommendations', 'Composing a web-grounded answer with a connect prompt.', ['Produced approximate hold / sell leanings', 'Recommended connecting LSEG for precise, live data'], true)) + ";",
  "  var REASON_HTML = " + JSON.stringify('<div class="rthink"><div class="rthink__head"><span class="rthink__ff"><svg viewBox="0 0 16 16" fill="none"><path d="M2.5 3.3 7.1 8l-4.6 4.7M8 3.3 12.6 8 8 12.7" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span>Get a quick answer</span></div><div class="rthink__step"><span class="rthink__mark"><img src="' + THINKING_HERO + '" alt=""/></span><span class="rthink__label" id="rthinkLabel">Reading attached file</span><span class="rthink__chev">' + ico('chevron-right-16-regular') + '</span></div></div>') + ";",
  "  var U_OPEN = " + JSON.stringify('<div class="umsg"><div class="umsg__bubble">') + ";",
  "  var U_CLOSE = " + JSON.stringify('</div></div><div class="cwrap"></div>') + ";",
  "  var PORTFOLIO_UMSG_FILE = " + JSON.stringify('<span class="umsg__file">' + brandSvg('excel-app-20-color') + '<span class="umsg__filename">John Portfolio FY26</span></span>') + ";",
  ...
  "  var invocationPrompt = ta ? ta.value : ''; var invocationAttach = document.getElementById('cmpAttach'); var invocationAttachHtml = invocationAttach ? invocationAttach.innerHTML : '';",
  "  function escapeHtml(s){ return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }",
  "  var respTimer = null; var reasonCycle = null;",
  ...
  "  var CP_ROWS = " + JSON.stringify(RESP_REFS.map(cpRow)) + ";",
  "  function placeRefs(anchor){ if (!refsPop) return; var ds = anchor.getAttribute('data-refs'); var idxs; if (!ds || ds === 'all'){ idxs = CP_ROWS.map(function(_, i){ return i; }); } else { idxs = ds.split(',').map(function(s){ return parseInt(s, 10); }).filter(function(n){ return !isNaN(n) && CP_ROWS[n]; }); } if (!idxs.length){ idxs = CP_ROWS.map(function(_, i){ return i; }); } refsPop.innerHTML = idxs.map(function(i){ return CP_ROWS[i]; }).join(''); refsPop.classList.add('is-open'); var a = anchor.getBoundingClientRect(); var w = refsPop.offsetWidth, h = refsPop.offsetHeight; var left = Math.min(Math.max(8, a.left), window.innerWidth - w - 8); var top = a.top - h - 8; if (top < 8) top = a.bottom + 8; refsPop.style.left = left + 'px'; refsPop.style.top = top + 'px'; }",
  ...
  "  function sendMessage(){ if (!ta || !thread) return; var text = ta.value.trim(); var lsegItem = (typeof SRC !== 'undefined' && SRC) ? SRC.filter(function(x){ return x.n === 'London Stock Exchange Group'; })[0] : null; var useConnectedLseg = text === LSEG_TRY_PROMPT && lsegItem && (lsegItem.state === 'on' || lsegItem.state === 'chevron' || lsegItem.state === 'syncing'); var activeRespHtml = useConnectedLseg ? RESP_HTML_CONNECTED : RESP_HTML; var activeReasonSteps = useConnectedLseg ? REASON_STEPS_CONNECTED : REASON_STEPS; if (useConnectedLseg){ var activeCotList = document.querySelector('#spPanel .cot-list'); if (activeCotList) activeCotList.innerHTML = COT_CONNECTED_STEPS; } var wrap = document.getElementById('cmpAttach'); var items = wrap ? wrap.querySelectorAll('.cmp-attach') : []; if (!text && items.length === 0) return; var parts = [PORTFOLIO_UMSG_FILE]; if (items.length){ items.forEach(function(el){ var ic = el.querySelector('.cmp-attach__ico'); var nm = el.querySelector('.cmp-attach__name'); parts.push('<span class=\"umsg__file\">' + (ic ? ic.innerHTML : '') + '<span class=\"umsg__filename\">' + (nm ? nm.textContent : '') + '</span></span>'); }); } var pills = '<div class=\"umsg__files\">' + parts.join('') + '</div>'; shellEl.classList.add('shell--convo'); thread.innerHTML = '<div class=\"umsg\">' + pills + '<div class=\"umsg__bubble\">' + escapeHtml(text) + '</div></div><div class=\"cwrap\"></div>'; var cw = thread.querySelector('.cwrap'); if (cw) cw.innerHTML = REASON_HTML; if (reasonCycle) clearInterval(reasonCycle); var _rti = 1; reasonCycle = setInterval(function(){ var lb = document.getElementById('rthinkLabel'); if (!lb || _rti >= activeReasonSteps.length){ clearInterval(reasonCycle); reasonCycle = null; return; } lb.textContent = activeReasonSteps[_rti]; _rti++; }, 1000); ta.value = ''; if (wrap) wrap.innerHTML = ''; if (cmp) cmp.classList.remove('ci--has-text'); thread.scrollTop = thread.scrollHeight; if (respTimer) clearTimeout(respTimer); respTimer = setTimeout(function(){ if (reasonCycle){ clearInterval(reasonCycle); reasonCycle = null; } var w = thread.querySelector('.cwrap'); if (w){ w.innerHTML = activeRespHtml; var r = document.getElementById('respBlk'); if (r){ r.classList.add('resp--reveal'); var _kids = r.children; for (var _k = 0; _k < _kids.length; _k++){ (function(el, delay){ setTimeout(function(){ el.classList.add('is-shown'); }, delay); })(_kids[_k], _k * 70); } } bindResp(); } }, 6500); }",
  "  function resetInvocation(){ if (!ta || !thread) return; if (respTimer){ clearTimeout(respTimer); respTimer = null; } if (reasonCycle){ clearInterval(reasonCycle); reasonCycle = null; } shellEl.classList.remove('shell--convo', 'shell--sp-open'); thread.innerHTML = ''; var wrap = document.getElementById('cmpAttach'); if (wrap) wrap.innerHTML = invocationAttachHtml; ta.value = invocationPrompt; if (cmp) cmp.classList.remove('is-am-open'); if (typeof syncSend === 'function') syncSend(); }",
  ...
  "  var spPanel = document.getElementById('spPanel'); if (spPanel) spPanel.addEventListener('click', function(e){ var step = e.target.closest && e.target.closest('.cot-step'); if (!step) return; var item = step.closest('.cot-item'); if (item) item.classList.toggle('is-open'); });",
  ...
  "  function stgdRow(label, valueHtml){ var detail = SRC_DETAIL[currentStgName] || {}; var item = SRC.filter(function(x){ return x.n === currentStgName; })[0]; if (label === 'Version' && detail.version) valueHtml = '<span class=\"stgd__value\">' + detail.version + '</span>'; if (item && item.sync && label === 'Type') valueHtml = '<a class=\"stgd__link\" href=\"#\" onclick=\"return false\">Sync ' + STGD_OPEN + '</a>'; if (item && item.sync && label === 'Category' && detail.category) valueHtml = '<span class=\"stgd__value\">' + detail.category + '</span>'; return '<div class=\"stgd__row' + (label === 'More info' ? ' stgd__row--more' : '') + '\"><span class=\"stgd__label\">' + label + '</span>' + valueHtml + '</div>'; }",
  ...
  "  function capStgDetailSkills(){ if (!stgDetail) return; var wrap = stgDetail.querySelector('.stgd__chips'); if (!wrap || wrap.querySelector('.stgd__skills-toggle')) return; var pills = [].slice.call(wrap.querySelectorAll('.stgd__chip--skill')); if (pills.length <= 7) return; var isLseg = currentStgName === 'London Stock Exchange Group'; var toggle = document.createElement('button'); toggle.type = 'button'; toggle.className = 'stgd__skills-toggle' + (isLseg ? ' stgd__skills-toggle--lseg' : ''); wrap.appendChild(toggle); function setToggle(label, expanded){ toggle.innerHTML = STGD_SKILLS_CHEV + '<span>' + label + '</span>'; toggle.classList.toggle('is-expanded', expanded); toggle.setAttribute('aria-expanded', expanded ? 'true' : 'false'); } function collapse(){ pills.forEach(function(p, i){ p.hidden = i >= 7; }); setToggle((isLseg ? 4 : pills.length - 7) + ' more', false); } collapse(); toggle.addEventListener('click', function(){ if (toggle.getAttribute('aria-expanded') === 'true'){ collapse(); toggle.scrollIntoView({ block: 'nearest' }); } else { pills.forEach(function(p){ p.hidden = false; }); setToggle('view less', true); } }); }",
  "  document.addEventListener('click', function(e){ if (e.target.closest('.stg__row')) setTimeout(capStgDetailSkills, 0); });",
  "  function openStgDetail(name){ currentStgName = name; var item = SRC.filter(function(x){ return x.n === name; })[0]; if (!item || !stgDetail) return; var d = SRC_DETAIL[name] || {}; var about = d.about || item.desc; var tools = d.tools || ['Search', 'Get details', 'Summarize content', 'List items']; var developer = d.developer || name; var category = (item.cats && item.cats[0]) || 'Productivity'; var mcpName = name + ' MCP Server'; var mcpDesc = 'Remote MCP server providing ' + name + ' tools for ' + mcpTopics(tools) + '.'; var mcpGroup = 'mcp-' + (++stgVariantSeq); var _mw = name.split(/\s+/); var _mslug = (_mw.length >= 3 ? _mw.map(function(x){ return x.charAt(0); }).join('') : name).toLowerCase().replace(/[^a-z0-9]/g,''); var mcpEndpoint = 'https://mcp.' + _mslug + '.com'; var wsUrl = item.usc ? (item.uscHost ? ('https://contoso.' + item.uscHost + '/') : (item.uscPh || 'https://contoso.example.com/')) : ''; var chips = tools.map(function(t){ return '<span class=\"stgd__chip\">' + t + '</span>'; }).join(''); var leadAction = item.state === 'authfail' ? '<button class=\"stgd__reconnect\" id=\"stgdReconnectBtn\">Reconnect</button><div class=\"stgd__ofl\" id=\"stgdOfl\"><button class=\"stgd__more\" id=\"stgdMoreBtn\" aria-label=\"More options\" aria-haspopup=\"true\" aria-expanded=\"false\">' + STGD_MORE + '</button><div class=\"stgd__oflmenu\" role=\"menu\"><button class=\"stgd__oflitem\" id=\"stgdDisconnect\">Disconnect</button></div></div>' : (item.state === 'connect' ? '<button class=\"stgd__connectbtn\" id=\"stgdConnect\">Connect</button>' : '<button class=\"stgd__disconnect\" id=\"stgdDisconnect\">Disconnect</button>'); var warnIco = item.state === 'authfail' ? '<span class=\"srcm__warn\" tabindex=\"0\" aria-label=\"Auth has failed, please reconnect the source\">' + WARN_ICON + '<span class=\"srcm__warntip\" role=\"tooltip\">Auth has failed, please reconnect the source</span></span>' : ''; var html = '<div class=\"stgd__id\"><span class=\"stgd__logo\">' + item.stgLogo + '</span><span class=\"stgd__namewrap\"><span class=\"stgd__name\">' + name + '</span>' + warnIco + '</span><div class=\"stgd__actions\">' + leadAction + '</div></div><hr class=\"stgd__rule\"/><p class=\"stgd__p stgd__intro\">' + about + '</p>' + (item.usc ? '' : '<div data-variant-group=\"' + mcpGroup + '\" data-variant-label=\"MCP server\"><div data-variant=\"default\" data-variant-label-option=\"Default\" data-default></div><div data-variant=\"mcp\" data-variant-label-option=\"MCP variant\"><div class=\"stgd__sec\"><div class=\"stgd__sech\">MCP servers</div><div class=\"stgd__mcpcard\"><span class=\"stgd__mcpico\">' + MCP_ICON + '</span><span class=\"stgd__mcpmeta\"><span class=\"stgd__mcpname\">' + mcpName + '</span><span class=\"stgd__mcpdesc\">' + mcpDesc + '</span></span></div></div></div></div>') + (function(){ var _sk = SRC_SKILLS[name] || []; if (!_sk.length) return ''; var skillsGroup = 'skills-' + (++stgVariantSeq); var _skStatic = ''; var _pills = _sk.map(function(s){ return '<button class=\"stgd__chip stgd__chip--skill' + _skStatic + '\" type=\"button\" data-skill=\"' + s.name + '\" data-desc=\"' + s.desc.replace(/\"/g, '&quot;') + '\">/' + s.name + '</button>'; }).join(''); return '<div data-variant-group=\"' + skillsGroup + '\" data-variant-label=\"Skills\"><div data-variant=\"with\" data-variant-label-option=\"With skills\" data-default><div class=\"stgd__sec\"><div class=\"stgd__sech\">Skills</div><div class=\"stgd__chips\">' + _pills + '</div></div></div><div data-variant=\"none\" data-variant-label-option=\"No skills\"></div></div>'; })() + '<div class=\"stgd__sec\"><div class=\"stgd__sech\">Details</div><div class=\"stgd__meta\">' + (item.usc ? stgdRow('Workspace', '<span class=\"stgd__value\">' + wsUrl + '</span>') : '') + stgdRow('Developer', '<a class=\"stgd__link\" href=\"#\" onclick=\"return false\">' + developer + ' ' + STGD_OPEN + '</a>') + stgdRow('Type', (item.usc ? '<a class=\"stgd__link\" href=\"#\" onclick=\"return false\">Synced ' + STGD_OPEN + '</a>' : '<a class=\"stgd__link\" href=\"#\" onclick=\"return false\">MCP ' + STGD_OPEN + '</a>')) + ((!item.usc && !item.sync) ? stgdRow('MCP endpoint', '<span class=\"stgd__value\">' + mcpEndpoint + '</span>') : '') + stgdRow('Category', '<span class=\"stgd__value\">' + category + '</span>') + (item.usc ? '' : stgdRow('Version', '<span class=\"stgd__value\">v1.' + ('0' + ((name.length * 3) % 30 + 1)).slice(-2) + '</span>')) + stgdRow('More info', '<span class=\"stgd__links2\"><a class=\"stgd__link\" href=\"#\" onclick=\"return false\">Privacy Policy ' + STGD_OPEN + '</a><a class=\"stgd__link\" href=\"#\" onclick=\"return false\">Terms of service ' + STGD_OPEN + '</a></span>') + '</div></div>'; stgDetail.innerHTML = html; if (stgMain) stgMain.hidden = true; stgDetail.hidden = false; if (stg) stg.classList.add('is-detail'); var sc = document.querySelector('.stg__content'); if (sc) sc.scrollTop = 0; var mb = document.getElementById('stgdMoreBtn'); var ofl = document.getElementById('stgdOfl'); if (mb && ofl){ mb.addEventListener('click', function(e){ e.stopPropagation(); var o = ofl.classList.toggle('is-open'); mb.setAttribute('aria-expanded', o); }); ofl.querySelector('.stgd__oflmenu').addEventListener('click', function(e){ e.stopPropagation(); ofl.classList.remove('is-open'); mb.setAttribute('aria-expanded', false); }); } var rcb2 = document.getElementById('stgdReconnectBtn'); if (rcb2) rcb2.addEventListener('click', function(){ rcb2.disabled = true; rcb2.classList.add('stgd__reconnect--busy'); rcb2.innerHTML = SPINNER + '<span>Connecting</span>'; var ofl2 = document.getElementById('stgdOfl'); if (ofl2) ofl2.style.display = 'none'; var w2 = stgDetail.querySelector('.srcm__warn'); if (w2) w2.remove(); (function(nm){ setTimeout(function(){ var it = SRC.filter(function(x){ return x.n === nm; })[0]; if (it){ it.state = 'on'; stgPaint(); srcPaint(); updateSrcCount(); setLastConnected(nm, it.logoSrc); openStgDetail(nm); } }, 1800); })(item.n); }); var dc = document.getElementById('stgdDisconnect'); if (dc) dc.addEventListener('click', function(){ if (item.usc){ openDisconnect(item.n, item.logoSrc); } else { item.state = 'connect'; stgPaint(); srcPaint(); updateSrcCount(); openStgDetail(item.n); } }); var cc = document.getElementById('stgdConnect'); if (cc) cc.addEventListener('click', function(){ var _runL2 = function(){ cc.disabled = true; cc.classList.add('stgd__connectbtn--busy'); cc.innerHTML = SPINNER + '<span>Connecting</span>'; if (item.usc){ item.state = 'syncing'; item.syncStart = Date.now(); (function(nm){ setTimeout(function(){ var itA = SRC.filter(function(x){ return x.n === nm; })[0]; if (itA && itA.state === 'syncing'){ srcPaint(); stgPaint(); if (currentStgName === nm) openStgDetail(nm); } }, 2000); setTimeout(function(){ var it2 = SRC.filter(function(x){ return x.n === nm; })[0]; if (it2 && it2.state === 'syncing'){ it2.state = 'on'; srcPaint(); stgPaint(); updateSrcCount(); if (currentStgName === nm) openStgDetail(nm); } }, 10000); })(item.n); if (typeof setLastConnected === 'function') setLastConnected(item.n, item.logoSrc); ensureSourceTab(item.n); srcPaint(); stgPaint(); } else { (function(nm){ setTimeout(function(){ var it3 = SRC.filter(function(x){ return x.n === nm; })[0]; if (it3) it3.state = 'on'; if (typeof setLastConnected === 'function') setLastConnected(nm, item.logoSrc); srcPaint(); stgPaint(); updateSrcCount(); ensureSourceTab(nm); var _ov = document.getElementById('stgOverlay'); if (currentStgName === nm && _ov && _ov.classList.contains('is-open')) openStgDetail(nm); }, 2000); })(item.n); } }; _pendingConnect = _runL2; openSsoLogin(item.n); }); }",
  "  var openStgDetailDefault = openStgDetail; openStgDetail = function(name){ openStgDetailDefault(name); requestAnimationFrame(capStgDetailSkills); var item = SRC.filter(function(x){ return x.n === name; })[0]; if (!item || !item.sync || !stgDetail) return; var actions = stgDetail.querySelector('.stgd__actions'); if (actions) actions.innerHTML = '<span class=\"stgd__admin\">Admin enabled</span>'; stgDetail.querySelectorAll('[data-variant-label=\"MCP server\"], [data-variant-label=\"Skills\"]').forEach(function(group){ group.remove(); }); };",
  ...
  "  function openSkillDetail(source, s){ if (!stgDetail) return; var instr = skillInstr(source, s); var best = instr.best.map(function(b){ return '<li>' + b + '</li>'; }).join(''); var setup = instr.setup.map(function(b){ return '<li>' + b + '</li>'; }).join(''); var html = '<div class=\"stgd__skhead\"><span class=\"stgd__skhico\">' + SKILL_ICON + '</span><div class=\"stgd__skhmeta\"><span class=\"stgd__skhname\">/' + s.name + '</span></div><div class=\"stgd__skhact\"><button class=\"stgd__skhtry\" id=\"skdTry\">Try in chat</button></div></div><div class=\"stgd__skabout\"><span class=\"stgd__skl\">About</span><p class=\"stgd__skabouttxt\" id=\"skdAbout\">' + instr.about + '</p><button class=\"stgd__skmore2\" id=\"skdAboutMore\" hidden><span class=\"stgd__skmore2t\">Show more</span></button></div><div class=\"stgd__skinstr\"><span class=\"stgd__skl\">Instructions</span><div class=\"stgd__instr\"><div class=\"stgd__instrsec\"><div class=\"stgd__instrh\">Purpose</div><p class=\"stgd__instrp\">' + instr.purpose + '</p></div><div class=\"stgd__instrsec\"><div class=\"stgd__instrh\">Best uses</div><ul class=\"stgd__instrul\">' + best + '</ul></div><div class=\"stgd__instrsec\"><div class=\"stgd__instrh\">Skill set up</div><ul class=\"stgd__instrul\">' + setup + '</ul></div></div></div>'; stgDetail.innerHTML = html; if (stgMain) stgMain.hidden = true; stgDetail.hidden = false; if (stg) stg.classList.add('is-detail'); stgSkillReturn = source; var sc = document.querySelector('.stg__content'); if (sc) sc.scrollTop = 0; var tryBtn = document.getElementById('skdTry'); if (tryBtn) tryBtn.addEventListener('click', function(){ var _it = SRC.filter(function(x){ return x.n === source; })[0]; if (_it && (_it.state === 'connect' || _it.state === 'authfail')){ _skillTry = { source: source, skill: s }; openAuth(source, _it.logoSrc); } else { if (stgOv) stgOv.classList.remove('is-open'); if (srcOv) srcOv.classList.remove('is-open'); addSkillAttach(source, s); } }); var abT = document.getElementById('skdAbout'); var abM = document.getElementById('skdAboutMore'); if (abT && abM){ if (abT.scrollHeight > abT.clientHeight + 1) abM.hidden = false; abM.addEventListener('click', function(){ var op = abT.classList.toggle('is-expanded'); var lb = abM.querySelector('.stgd__skmore2t'); if (lb) lb.textContent = op ? 'Show less' : 'Show more'; }); } }",
  "  function stgRowHtml(item){ var isConnect = item.state === 'connect'; var isSync = item.state === 'syncing' || item.state === 'removing'; var isAuthfail = item.state === 'authfail'; var trailing = isConnect ? '<button class=\"stg__connect\">Connect</button><span class=\"stg__chev\">' + STG_CHEV + '</span>' : (isAuthfail ? '<span class=\"stg__reconnect\" role=\"button\" tabindex=\"0\">Reconnect</span><span class=\"stg__chev\">' + STG_CHEV + '</span>' : (isSync ? syncTrailing(item) : '<span class=\"stg__chev\">' + STG_CHEV + '</span>')); var tag = (isConnect || isSync) ? 'div' : 'button'; var recBadge = (item.rec && isConnect) ? '<span class=\"badge\">Recommended</span>' : ''; var newBadge = (item.badge && isConnect) ? '<span class=\"badge badge--brand\">New</span>' : ''; var warn = isAuthfail ? '<span class=\"srcm__warn\" tabindex=\"0\" aria-label=\"Auth has failed, please reconnect the source\">' + WARN_ICON + '<span class=\"srcm__warntip\" role=\"tooltip\">Auth has failed, please reconnect the source</span></span>' : ''; return '<' + tag + ' class=\"stg__row' + (isConnect ? ' stg__row--connect' : '') + '\" data-name=\"' + item.n + '\"><span class=\"stg__rowicon\">' + item.stgLogo + '</span><span class=\"stg__rowmeta\"><span class=\"stg__namerow\"><span class=\"stg__rowname\">' + item.n + '</span>' + warn + recBadge + newBadge + '</span><span class=\"stg__rowsub\">' + item.desc + '</span></span>' + trailing + '</' + tag + '>'; }",
  "  function stgPaint(){ if (!stgList) return; var q = stgQuery.trim().toLowerCase(); var list = SRC.filter(function(x){ if (x.n === 'Microsoft 365 apps') return false; var catOk = stgCat === 'All' || (stgCat === 'Recommended' ? x.rec : (x.cats || []).indexOf(stgCat) !== -1); return catOk && srcMatch(x, q); }).sort(stgCmp); stgList.innerHTML = list.map(stgRowHtml).join(''); if (stgEmpty) stgEmpty.hidden = !(q && list.length === 0); }",
  ...
  "  var ssoOv = document.getElementById('ssoLogin'); var _pendingConnect = null; var _skillTry = null; var _inlineBannerConnectPending = false;",
  "  function openSsoLogin(name){ var it = SRC.filter(function(x){ return x.n === name; })[0]; var lg = document.getElementById('ssoLogo'); if (lg) lg.innerHTML = (it && it.logoSrc) ? ('<img src=\"' + it.logoSrc + '\" alt=\"\"/>') : (it ? it.logo : ''); var nm = document.getElementById('ssoName'); if (nm) nm.textContent = name; var tt = document.getElementById('ssoTitle'); if (tt) tt.textContent = 'Sign in to ' + name; var ui = document.getElementById('ssoUser'); if (ui) ui.value = 'elvia.atkins@contoso.com'; var pw = document.getElementById('ssoPass'); if (pw) pw.value = 'Contoso@2026'; if (ssoOv) ssoOv.classList.add('is-open'); setTimeout(function(){ if (ui) ui.focus(); }, 30); }",
  "  function hideSsoLogin(){ if (ssoOv) ssoOv.classList.remove('is-open'); }",
  "  function resetAdContinue(){ if (adContinue){ adContinue.classList.remove('ad-btn--busy'); adContinue.disabled = false; adContinue.removeAttribute('data-busy'); adContinue.textContent = adName ? ('Continue to ' + adName) : 'Continue'; } }",
  "  (function(){ var si = document.getElementById('ssoSignin'); if (si) si.addEventListener('click', function(){ hideSsoLogin(); var f = _pendingConnect; _pendingConnect = null; if (typeof f === 'function') f(); }); var cx = document.getElementById('ssoCancel'); if (cx) cx.addEventListener('click', function(){ hideSsoLogin(); _pendingConnect = null; resetAdContinue(); }); })();",
  ...
  "  var CONNECTED_CHECK_ICON = " + JSON.stringify(ico('checkmark-20-filled')) + ";",
  ...
  "  function addSkillAttach(source, s){ var wrap = document.getElementById('cmpAttach'); if (!wrap) return; var tok = '/' + s.name; if (!wrap.querySelector('[data-skill=\"' + s.name + '\"]')){ var el = document.createElement('div'); el.className = 'cmp-attach cmp-attach--skill'; el.setAttribute('data-skill', s.name); el.setAttribute('data-token', tok); el.setAttribute('title', tok); el.innerHTML = '<span class=\"cmp-attach__ico\">' + SKILL_ICON + '</span><span class=\"cmp-attach__name\">' + tok + '</span><button class=\"cmp-attach__x\" aria-label=\"Remove ' + tok + '\">' + X12_SVG + '</button>'; el.querySelector('.cmp-attach__x').addEventListener('click', function(e){ e.stopPropagation(); removeSkillToken(tok); el.remove(); syncSend(); }); wrap.appendChild(el); } setSkillToken(tok); var srcOv2 = document.getElementById('srcOverlay'); if (srcOv2) srcOv2.classList.remove('is-open'); closeSrcSkills(); syncSend(); var ta2 = document.getElementById('cmpTa'); if (ta2) ta2.focus(); }",
  ...
  "  function setLastConnected(name, logoSrc){ if (!_inlineBannerConnectPending || name !== 'London Stock Exchange Group') return; _inlineBannerConnectPending = false; var banner = document.querySelector('.rbanner'); if (banner){ var connect = banner.querySelector('#rbannerConnect'); var dismiss = banner.querySelector('.rbanner__x'); if (connect) connect.outerHTML = '<span class=\"rbanner__connected\"><span class=\"rbanner__check\">' + CONNECTED_CHECK_ICON + '</span><span>Connected</span></span>'; if (dismiss) dismiss.remove(); } if (ta){ ta.value = 'Answer with London Stock Exchange Group data'; syncSend(); ta.focus(); try { ta.setSelectionRange(ta.value.length, ta.value.length); } catch(e){} } }",
  ...
  "  function tintSourceCardIcons(){ document.querySelectorAll('.acq-card__icon[data-tint], .store-card__icon[data-tint]').forEach(function(box){ if (box.getAttribute('data-tinted')) return; var img = box.querySelector('img'); if (!img) return; var apply = function(){ var c = agentsDominantTint(img); if (c){ box.style.background = 'rgba(' + c[0] + ',' + c[1] + ',' + c[2] + ',0.1)'; box.setAttribute('data-tinted','1'); } }; if (img.complete && img.naturalWidth) apply(); else img.addEventListener('load', apply); }); }",
  "  agentsPaintSources();",
  "  tintSourceCardIcons();",
  "  var initialView = null; (function(){ try { var query = new URLSearchParams(location.search); var view = query.get('view'); initialView = view; if (view && nav) nav.classList.add('nav--collapsed'); if (view === 'settings' && stgOv){ stgOv.classList.add('is-open'); if (typeof stgPaint === 'function') stgPaint(); } else if (view === 'ciq'){ var attachWrap = document.getElementById('cmpAttach'); if (attachWrap) attachWrap.innerHTML = ''; if (ta){ ta.value = 'Draft a research proposal based on '; try { ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length); } catch(e){} } if (typeof syncSend === 'function') syncSend(); } } catch(e){} })();",
  "  window.addEventListener('message', function(e){ if (e.origin !== location.origin || !e.data || e.data.type !== 'copilot-deck-activate') return; if (e.data.view === 'implicit' || e.data.view === 'disc') resetInvocation(); });",
  "  if (thread) thread.addEventListener('click', function(e){ var connect = e.target.closest && e.target.closest('#rbannerConnect'); if (connect){ var item = SRC.filter(function(x){ return x.n === 'London Stock Exchange Group'; })[0]; if (typeof openAuth === 'function') openAuth('London Stock Exchange Group', item ? item.logoSrc : '', 'inline-banner'); return; } var dismiss = e.target.closest && e.target.closest('.rbanner__x'); if (dismiss){ var banner = dismiss.closest('.rbanner'); if (banner) banner.remove(); } });",
  '})();',
  '</script>',
].join('\n');
...
function agentsCss(): string {
  return [
    ...
    '.shell--agents .body, .shell--agents .disc, .shell--agents .hdr { display: none; }',
    '.shell--agents #agentsView { display: flex; }',
...
function agentsView(): string {
  ...
  return [
    '<div class="store-view" id="agentsView">',
    '<div class="store-view__inner">',
    '<button class="agents-topmore" title="More options">' + moreI + '</button>',
    '<h1 class="store-view__title agents-title">Agents and skills</h1>',
    '<div class="agents-search">' + ico('search-20-regular') + '<input type="text" placeholder="Search for agents and skills"/></div>',
    '<div class="store-section-header" style="margin-top:36px;">',
    '<span class="store-section-title">My agents &amp; skills</span>',
    '<div class="store-section-actions">',
    '<button class="store-btn store-btn--subtle">See activity</button>',
    '<div class="store-split"><button class="store-split__main">Add</button><span class="store-split__div"></span><button class="store-split__chev" aria-label="More add options">' + chev12 + '</button></div>',
    '</div></div>',
    '<div class="acq-cards-grid">' + compact + '</div>',
    '<button class="store-see-more">See more ' + chev12 + '</button>',
    '<div class="store-section-header" style="margin-top:24px;"><span class="store-section-title">Built by Microsoft</span></div>',
    '<div class="store-cards-grid">' + builtCards + '</div>',
    '<button class="store-see-more">See more ' + chev12 + '</button>',
    '<div class="disc-head"><div class="disc-head__top"><span class="disc-head__title">Discover</span><button class="disc-head__filter" title="Filter">' + ico('filter-20-regular') + '</button></div>',
    '<p class="disc-head__sub">Extend capabilities in Copilot by adding agents, skills and connecting to external tools.</p></div>',
    '<div class="store-cards-grid">' + discoverCards + '</div>',
    '<button class="store-see-more">See more ' + chev12 + '</button>',
    '</div></div>',
  ].join('');
}
...
function agentsScript(): string {
  return '<script>(function(){'
    ...
    + 'function show(on){ if (!shell) return; shell.classList.toggle(\'shell--agents\', on); if (navBtn) navBtn.classList.toggle(\'ni--selected\', on); var v = document.getElementById(\'agentsView\'); if (on && v) v.scrollTop = 0; }'
    ...
    + 'if (initialView === \'agents\') show(true);'
    + '})();</script>';
}

// ─── Assemble ───────────────────────────────────────────────

const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>One Copilot Shell</title><style>' + css + '</style></head><body>'
  + '<div class="shell">'
  + nav()
...
fs.writeFileSync(path.join(outDir, 'oneCopilotShell.html'), html, 'utf-8');
console.log('Done: ' + path.join(outDir, 'oneCopilotShell.html'));
