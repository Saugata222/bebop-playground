/**
 * Nav — Interactive HTML Preview
 *
 * Shows the full nav shell in expanded and collapsed modes,
 * plus individual sub-components: nav items, split items,
 * section headers, header buttons, me control.
 */

import * as fs from 'fs';
import * as path from 'path';

// ─── Icons (20px regular) — Fluent UI System Icons ──────────

const copilotIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.92704 2H12.1392C13.1927 2 14.1203 2.69397 14.4176 3.7046L14.8868 5.29901C15.0476 5.84546 15.5266 6.232 16.086 6.28068H16.3751C17.2627 6.28068 17.9479 6.53232 18.4023 7.04832C18.8449 7.55084 18.9913 8.21838 19.0009 8.88707C19.02 10.208 18.5033 11.849 18.0916 13.1554C17.7333 14.2924 17.273 15.4729 16.6552 16.3777C16.0399 17.2788 15.1999 18.0006 14.073 18.0006H7.87484L7.86695 18.0005H7.86063C6.80716 18.0005 5.87959 17.3066 5.58222 16.296L5.11309 14.7015C4.95237 14.1553 4.47368 13.7689 3.91456 13.7199H3.62488C2.73727 13.7199 2.05209 13.4683 1.59766 12.9523C1.15509 12.4498 1.00872 11.7822 0.999049 11.1135C0.979951 9.7926 1.49665 8.15165 1.90838 6.84517C2.26667 5.70826 2.72694 4.52772 3.34477 3.62288C3.96005 2.72178 4.80007 2 5.92704 2ZM2.86214 7.14574C2.43465 8.50224 1.98267 9.97352 1.99894 11.0991C2.00696 11.6536 2.12929 12.0429 2.34812 12.2914C2.55509 12.5264 2.924 12.7199 3.62488 12.7199H6.25213C6.8614 12.7199 7.39814 12.3195 7.57212 11.7352C8.04172 10.158 8.83794 7.4963 9.46637 5.45934L9.49937 5.35233C9.64852 4.86841 9.79284 4.40016 9.94826 3.98475C10.0807 3.63087 10.2289 3.29335 10.4095 3H5.92704C5.28311 3 4.70642 3.40208 4.17062 4.18678C3.63737 4.96774 3.2122 6.03493 2.86214 7.14574ZM5.73445 13.7199C5.88192 13.9288 5.99729 14.1639 6.07243 14.4193L6.54156 16.0137C6.71371 16.5988 7.25073 17.0005 7.86063 17.0005H7.88678C8.24574 16.9969 8.48955 16.8275 8.64066 16.6226C8.80779 16.3959 8.96035 16.0792 9.11514 15.6654C9.25974 15.2789 9.39567 14.8381 9.5478 14.3446L9.57806 14.2465C9.64307 14.0357 9.70989 13.8183 9.77797 13.596C9.543 13.6733 9.29325 13.7147 9.03611 13.7147H6.41074C6.35823 13.7182 6.30534 13.7199 6.25213 13.7199H5.73445ZM8.1935 12.7147H9.03611C9.58916 12.7147 10.0832 12.3842 10.2986 11.8846C10.7359 10.4387 11.1692 8.98844 11.4694 7.98005C11.5449 7.72668 11.6599 7.4933 11.8065 7.28588H10.9638C10.4111 7.28588 9.91739 7.61592 9.70178 8.11487C9.26432 9.56118 8.83086 11.0119 8.53054 12.0206C8.45511 12.2739 8.34011 12.5073 8.1935 12.7147ZM10.2221 6.40449C10.457 6.32723 10.7067 6.28588 10.9638 6.28588H13.5902C13.6424 6.28243 13.695 6.28068 13.7479 6.28068H14.2655C14.118 6.07183 14.0026 5.83666 13.9274 5.58128L13.4583 3.98687C13.2862 3.40177 12.7491 3 12.1392 3H12.1244C11.7593 3.00023 11.512 3.17099 11.3593 3.37803C11.1922 3.60467 11.0396 3.92146 10.8848 4.33517C10.7402 4.72167 10.6043 5.16256 10.4522 5.65601L10.4219 5.75415C10.3569 5.96483 10.2901 6.18225 10.2221 6.40449ZM17.1379 12.8549C17.5653 11.4984 18.0173 10.0271 18.001 8.90153C17.993 8.34698 17.8707 7.95771 17.6519 7.70923C17.4449 7.47423 17.076 7.28068 16.3751 7.28068H13.7479C13.1386 7.28068 12.6018 7.68108 12.4279 8.26541C11.9583 9.84263 11.1621 12.5043 10.5336 14.5413L10.5006 14.6483C10.3515 15.1322 10.2072 15.6005 10.0517 16.0159C9.91933 16.3697 9.77114 16.7073 9.59044 17.0006H14.073C14.7169 17.0006 15.2936 16.5985 15.8294 15.8138C16.3626 15.0329 16.7878 13.9657 17.1379 12.8549Z" fill="currentColor"/></svg>';
const pinIco20 = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.1221 3.13715C10.7326 1.91616 12.3599 1.65208 13.3251 2.61737L17.382 6.67419C18.3472 7.63947 18.0832 9.26676 16.8622 9.87726L13.4037 11.6065C13.0751 11.7708 12.8183 12.0499 12.6818 12.391L11.2459 15.981C10.9792 16.6476 10.1179 16.8244 9.61027 16.3167L7 13.7064L3.70711 16.9993H3V16.2922L6.29289 12.9993L3.68262 10.3891C3.17498 9.88142 3.35177 9.02011 4.01834 8.75348L7.60829 7.3175C7.94939 7.18106 8.22855 6.92419 8.39285 6.5956L10.1221 3.13715ZM12.618 3.32447C12.1354 2.84183 11.3217 2.97387 11.0165 3.58437L9.28727 7.04282C9.01345 7.59046 8.54818 8.01858 7.97968 8.24598L4.38973 9.68196L10.3174 15.6096L11.7534 12.0197C11.9808 11.4512 12.4089 10.9859 12.9565 10.7121L16.415 8.98283C17.0255 8.67758 17.1575 7.86394 16.6749 7.3813L12.618 3.32447Z" fill="currentColor"/></svg>';
const composeIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.5 4.00003C10.7761 4.00003 11 4.22389 11 4.50003C10.9999 4.77614 10.7761 5.00003 10.5 5.00003H6C4.89543 5.00003 4 5.89546 4 7.00002V14C4.00004 15.1045 4.89545 16 6 16H13C14.1045 16 14.9999 15.1045 15 14V9.50002C15 9.22388 15.2238 9.00002 15.5 9.00002C15.7761 9.00002 16 9.22388 16 9.50002V14C15.9999 15.6568 14.6568 17 13 17H6C4.34317 17 3.00004 15.6568 3 14V7.00002C3 5.34318 4.34314 4.00003 6 4.00003H10.5ZM16.1465 3.14652C16.3417 2.95126 16.6582 2.95126 16.8535 3.14652C17.0487 3.34179 17.0487 3.65832 16.8535 3.85355L9.06054 11.6455L7.99999 12L8.35351 10.9395L16.1465 3.14652Z" fill="currentColor"/></svg>';
const searchIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.7291 14.4362C12.5924 15.411 11.115 16 9.5 16C5.91015 16 3 13.0899 3 9.5C3 5.91015 5.91015 3 9.5 3C13.0899 3 16 5.91015 16 9.5C16 11.115 15.411 12.5924 14.4361 13.7292L17.8535 17.1465C18.0487 17.3417 18.0487 17.6583 17.8535 17.8536C17.6799 18.0271 17.4105 18.0464 17.2156 17.9114L17.1464 17.8536L13.7291 14.4362ZM13.0196 13.7266C13.276 13.5128 13.5127 13.2761 13.7265 13.0197C14.5216 12.0659 15 10.8388 15 9.5C15 6.46243 12.5376 4 9.5 4C6.46243 4 4 6.46243 4 9.5C4 12.5376 6.46243 15 9.5 15C10.8388 15 12.0658 14.5217 13.0196 13.7266Z" fill="currentColor"/></svg>';
const agentIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.20949 2.81648C5.53001 2.30825 6.08885 2 6.6897 2H9.50021C9.77635 2 10.0002 2.22386 10.0002 2.5C10.0002 2.77614 9.77635 3 9.50021 3H6.6897C6.43219 3 6.19269 3.13211 6.05533 3.34992L2.11367 9.59992C1.95951 9.84436 1.95951 10.1556 2.11367 10.4001L5.97726 16.5263C6.16322 16.8212 6.48744 17 6.83604 17C7.29602 17 7.69854 16.6908 7.81706 16.2463L11.218 3.49284C11.4527 2.61252 12.25 2 13.1611 2C13.8557 2 14.5011 2.35842 14.8682 2.94804L18.7888 9.24456C19.0768 9.70702 19.0768 10.293 18.7888 10.7554L14.7916 17.175C14.4721 17.6881 13.9105 18 13.306 18H10.5002C10.2241 18 10.0002 17.7761 10.0002 17.5C10.0002 17.2239 10.2241 17 10.5002 17H13.306C13.5651 17 13.8058 16.8663 13.9427 16.6464L17.9399 10.2269C18.0264 10.088 18.0264 9.91202 17.9399 9.77313L14.0193 3.47661C13.8347 3.18019 13.5103 3 13.1611 3C12.703 3 12.3022 3.30794 12.1842 3.75051L8.78329 16.504C8.54804 17.3862 7.74907 18 6.83604 18C6.1441 18 5.50054 17.645 5.13143 17.0597L1.26783 10.9335C0.908119 10.3632 0.908119 9.63685 1.26783 9.06648L5.20949 2.81648Z" fill="currentColor"/></svg>';
const folderIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.5 3C3.11929 3 2 4.11929 2 5.5V14.5C2 15.8807 3.11929 17 4.5 17H15.5C16.8807 17 18 15.8807 18 14.5V7.5C18 6.11929 16.8807 5 15.5 5H9.70711L8.21967 3.51256C7.89148 3.18437 7.44636 3 6.98223 3H4.5ZM3 5.5C3 4.67157 3.67157 4 4.5 4H6.98223C7.18115 4 7.37191 4.07902 7.51256 4.21967L8.79289 5.5L7.43934 6.85355C7.34557 6.94732 7.21839 7 7.08579 7H3V5.5ZM3 8H7.08579C7.48361 8 7.86514 7.84196 8.14645 7.56066L9.70711 6H15.5C16.3284 6 17 6.67157 17 7.5V14.5C17 15.3284 16.3284 16 15.5 16H4.5C3.67157 16 3 15.3284 3 14.5V8Z" fill="currentColor"/></svg>';
const chatIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10 2C14.4183 2 18 5.58172 18 10C18 14.4183 14.4183 18 10 18C8.72679 18 7.49591 17.7018 6.38669 17.1393L6.266 17.075L2.62109 17.9851C2.31127 18.0625 2.02622 17.8369 2.00131 17.5438L2.00114 17.4624L2.01493 17.3787L2.925 13.735L2.86169 13.6153C2.4066 12.7186 2.12433 11.7422 2.03275 10.7283L2.00738 10.3463L2 10C2 5.58172 5.58172 2 10 2ZM10 3C6.13401 3 3 6.13401 3 10C3 11.217 3.31054 12.3878 3.89352 13.4249C3.94046 13.5084 3.9621 13.603 3.95692 13.6973L3.94274 13.7912L3.187 16.812L6.21104 16.0583C6.27294 16.0429 6.33662 16.0396 6.39873 16.0479L6.4903 16.0691L6.57701 16.1075C7.61362 16.6898 8.7837 17 10 17C13.866 17 17 13.866 17 10C17 6.13401 13.866 3 10 3ZM10.5 11C10.7761 11 11 11.2239 11 11.5C11 11.7455 10.8231 11.9496 10.5899 11.9919L10.5 12H7.5C7.22386 12 7 11.7761 7 11.5C7 11.2545 7.17688 11.0504 7.41012 11.0081L7.5 11H10.5ZM12.5 8C12.7761 8 13 8.22386 13 8.5C13 8.74546 12.8231 8.94961 12.5899 8.99194L12.5 9H7.5C7.22386 9 7 8.77614 7 8.5C7 8.25454 7.17688 8.05039 7.41012 8.00806L7.5 8H12.5Z" fill="currentColor"/></svg>';
const alertIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.99766 2C13.1466 2 15.7416 4.33488 15.9821 7.3554L15.9955 7.57762L16 7.80214L15.999 11.398L16.9244 13.6202C16.947 13.6743 16.9647 13.7302 16.9774 13.7871L16.9926 13.8733L17.0013 14.0046C17.0013 14.4526 16.7048 14.8387 16.2521 14.9677L16.1358 14.9945L16.0013 15.0046L12.4996 15.004L12.4946 15.1653C12.4095 16.469 11.3252 17.5 10 17.5C8.67453 17.5 7.58998 16.4685 7.50533 15.1644L7.49962 15.004L3.99891 15.0046C3.91096 15.0046 3.82358 14.993 3.73902 14.9702L3.61456 14.9277C3.20378 14.7567 2.96181 14.3392 3.01221 13.8757L3.0333 13.7483L3.07572 13.6202L3.99902 11.401L4.0001 7.79281L4.0044 7.56824C4.12702 4.45115 6.77104 2 9.99766 2ZM11.4996 15.004H8.49962L8.50697 15.1454C8.57552 15.8581 9.14275 16.425 9.85556 16.4931L10 16.5C10.7797 16.5 11.4205 15.9051 11.4931 15.1445L11.4996 15.004ZM9.99766 3C7.37511 3 5.22717 4.92372 5.01715 7.38498L5.00393 7.59723L5.00002 7.80214V11.5L4.96161 11.6922L3.9989 14.0046L15.9566 14.0066L16.0019 14.0045L15.0384 11.6922L15 11.5L15.0001 7.81241L14.996 7.60831C14.8909 5.0349 12.6947 3 9.99766 3Z" fill="currentColor"/></svg>';
const waffleIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5.25 4C5.25 4.69036 4.69036 5.25 4 5.25C3.30964 5.25 2.75 4.69036 2.75 4C2.75 3.30964 3.30964 2.75 4 2.75C4.69036 2.75 5.25 3.30964 5.25 4ZM17.25 16C17.25 16.6904 16.6904 17.25 16 17.25C15.3096 17.25 14.75 16.6904 14.75 16C14.75 15.3096 15.3096 14.75 16 14.75C16.6904 14.75 17.25 15.3096 17.25 16ZM16 11.25C16.6904 11.25 17.25 10.6904 17.25 10C17.25 9.30964 16.6904 8.75 16 8.75C15.3096 8.75 14.75 9.30964 14.75 10C14.75 10.6904 15.3096 11.25 16 11.25ZM17.25 4C17.25 4.69036 16.6904 5.25 16 5.25C15.3096 5.25 14.75 4.69036 14.75 4C14.75 3.30964 15.3096 2.75 16 2.75C16.6904 2.75 17.25 3.30964 17.25 4ZM10 17.25C10.6904 17.25 11.25 16.6904 11.25 16C11.25 15.3096 10.6904 14.75 10 14.75C9.30964 14.75 8.75 15.3096 8.75 16C8.75 16.6904 9.30964 17.25 10 17.25ZM11.25 10C11.25 10.6904 10.6904 11.25 10 11.25C9.30964 11.25 8.75 10.6904 8.75 10C8.75 9.30964 9.30964 8.75 10 8.75C10.6904 8.75 11.25 9.30964 11.25 10ZM10 5.25C10.6904 5.25 11.25 4.69036 11.25 4C11.25 3.30964 10.6904 2.75 10 2.75C9.30964 2.75 8.75 3.30964 8.75 4C8.75 4.69036 9.30964 5.25 10 5.25ZM5.25 16C5.25 16.6904 4.69036 17.25 4 17.25C3.30964 17.25 2.75 16.6904 2.75 16C2.75 15.3096 3.30964 14.75 4 14.75C4.69036 14.75 5.25 15.3096 5.25 16ZM4 11.25C4.69036 11.25 5.25 10.6904 5.25 10C5.25 9.30964 4.69036 8.75 4 8.75C3.30964 8.75 2.75 9.30964 2.75 10C2.75 10.6904 3.30964 11.25 4 11.25Z" fill="currentColor"/></svg>';
const moreIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.25 10C6.25 10.6904 5.69036 11.25 5 11.25C4.30964 11.25 3.75 10.6904 3.75 10C3.75 9.30964 4.30964 8.75 5 8.75C5.69036 8.75 6.25 9.30964 6.25 10ZM11.25 10C11.25 10.6904 10.6904 11.25 10 11.25C9.30964 11.25 8.75 10.6904 8.75 10C8.75 9.30964 9.30964 8.75 10 8.75C10.6904 8.75 11.25 9.30964 11.25 10ZM15 11.25C15.6904 11.25 16.25 10.6904 16.25 10C16.25 9.30964 15.6904 8.75 15 8.75C14.3096 8.75 13.75 9.30964 13.75 10C13.75 10.6904 14.3096 11.25 15 11.25Z" fill="currentColor"/></svg>';
const pinIco16 = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.0589 2.44535C9.34701 1.73087 8.14697 1.90854 7.67261 2.79864L5.6526 6.58902L2.8419 7.52592C2.6775 7.58072 2.5532 7.71673 2.51339 7.88539C2.47357 8.05404 2.52392 8.23128 2.64646 8.35382L4.79291 10.5003L2.14645 13.1467L2 14.0003L2.85356 13.8538L5.50002 11.2074L7.64646 13.3538C7.76899 13.4764 7.94623 13.5267 8.11489 13.4869C8.28354 13.4471 8.41955 13.3228 8.47435 13.1584L9.41143 10.3472L13.1897 8.32448C14.0759 7.85006 14.2538 6.65535 13.5443 5.9433L10.0589 2.44535ZM8.55511 3.26895C8.71323 2.97225 9.11324 2.91303 9.35055 3.15119L12.836 6.64914C13.0725 6.88648 13.0131 7.28472 12.7178 7.44286L8.76403 9.55946C8.65137 9.61977 8.56608 9.72092 8.52567 9.84215L7.7815 12.0746L3.92562 8.21877L6.15812 7.47461C6.27966 7.43409 6.38101 7.34848 6.44126 7.23542L8.55511 3.26895Z" fill="currentColor"/></svg>';
const chevDn = '<svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.14645 4.64645C2.34171 4.45118 2.65829 4.45118 2.85355 4.64645L6 7.79289L9.14645 4.64645C9.34171 4.45118 9.65829 4.45118 9.85355 4.64645C10.0488 4.84171 10.0488 5.15829 9.85355 5.35355L6.35355 8.85355C6.15829 9.04882 5.84171 9.04882 5.64645 8.85355L2.14645 5.35355C1.95118 5.15829 1.95118 4.84171 2.14645 4.64645Z" fill="currentColor"/></svg>';

// Arrow Sync 16px (status: in-progress)
const arrowSync16 = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.14645 0.646447C7.34171 0.451184 7.65829 0.451184 7.85355 0.646447L9.35355 2.14645C9.54882 2.34171 9.54882 2.65829 9.35355 2.85355L7.85355 4.35355C7.65829 4.54882 7.34171 4.54882 7.14645 4.35355C6.95118 4.15829 6.95118 3.84171 7.14645 3.64645L7.7885 3.00439C5.12517 3.11522 3 5.30943 3 8C3 9.56799 3.72118 10.9672 4.85185 11.8847C5.06627 12.0587 5.09904 12.3736 4.92503 12.588C4.75103 12.8024 4.43615 12.8352 4.22172 12.6612C2.86712 11.5619 2 9.88205 2 8C2 4.75447 4.57689 2.1108 7.79629 2.00339L7.14645 1.35355C6.95118 1.15829 6.95118 0.841709 7.14645 0.646447ZM11.075 3.41199C11.249 3.19756 11.5639 3.1648 11.7783 3.3388C13.1329 4.43806 14 6.11795 14 8C14 11.2455 11.4231 13.8892 8.20371 13.9966L8.85355 14.6464C9.04882 14.8417 9.04882 15.1583 8.85355 15.3536C8.65829 15.5488 8.34171 15.5488 8.14645 15.3536L6.64645 13.8536C6.55268 13.7598 6.5 13.6326 6.5 13.5C6.5 13.3674 6.55268 13.2402 6.64645 13.1464L8.14645 11.6464C8.34171 11.4512 8.65829 11.4512 8.85355 11.6464C9.04882 11.8417 9.04882 12.1583 8.85355 12.3536L8.2115 12.9956C10.8748 12.8848 13 10.6906 13 8C13 6.43201 12.2788 5.03283 11.1482 4.1153C10.9337 3.94129 10.901 3.62641 11.075 3.41199Z" fill="currentColor"/></svg>';

// ─── CSS ────────────────────────────────────────────────────

const css = `
*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
body { font-family: 'Segoe UI', 'Segoe Sans', system-ui, sans-serif; background: #f3f3f3; padding: 40px 24px; color: #242424; }
h1 { font-size: 24px; font-weight: 600; text-align: center; margin-bottom: 8px; }
h2 { font-size: 18px; font-weight: 600; margin: 40px 0 16px; border-bottom: 1px solid #e0e0e0; padding-bottom: 8px; }
h3 { font-size: 14px; font-weight: 600; margin: 20px 0 12px; color: #5d5d5d; }
.hint { font-size: 13px; color: #6f6f6f; text-align: center; margin-bottom: 32px; }
.wrap { max-width: 1100px; margin: 0 auto; }
.row { display: flex; gap: 32px; flex-wrap: wrap; margin-bottom: 16px; align-items: flex-start; }
.cell { display: flex; flex-direction: column; align-items: flex-start; gap: 6px; }
.rl { font-size: 10px; font-weight: 600; color: #6f6f6f; text-transform: uppercase; letter-spacing: 0.8px; }

/* ─── Bebop scrollbar ─── */
::-webkit-scrollbar { width: 6px; background: transparent; }
::-webkit-scrollbar-thumb { background: #6f6f6f; border-radius: 9999px; border: 2px solid transparent; background-clip: content-box; }
::-webkit-scrollbar-thumb:hover { border-width: 0; }
::-webkit-scrollbar-track { background: transparent; }

/* ─── Nav shell ─── */
.nav { background: #fff; display: flex; flex-direction: column; border-radius: 0; overflow: hidden; height: 700px; transition: width 0.25s ease; }
.nav--expanded { width: 256px; padding: 8px; }
.nav--collapsed { width: 56px; padding: 8px; align-items: center; }
.nav__header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px; transition: flex-direction 0.2s; }
.nav--collapsed .nav__header { flex-direction: column; gap: 4px; margin-bottom: 8px; }
.nav__body { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 0px; }
.nav--collapsed .nav__body { align-items: center; }
.nav__footer { margin-top: auto; padding-top: 8px; display: flex; align-items: center; }
.nav--collapsed .nav__footer { align-items: center; display: flex; flex-direction: column-reverse; gap: 4px; }

/* Collapsible hide/show */
.nav--collapsed .ni__title, .nav--collapsed .ni__right, .nav--collapsed .nsh, .nav--collapsed .ni-split__sec { display: none; }
.nav--collapsed .ni { width: 44px; padding: 8px 12px; justify-content: center; }
.nav--collapsed .ni-split { width: 44px; justify-content: center; }
.nav--collapsed .ni-split .ni { width: 44px; }
.nav--collapsed .me .me__info { display: none; }
.nav--collapsed .me { width: 44px; padding: 8px; justify-content: center; }
.nav--collapsed .nav__waffle { display: none; }
.nav--collapsed .nav__section--pinned { display: none; }
.nav--collapsed .nav__section--chats { display: none; }
.ni.nav__pin-collapsed { display: none; }
.nav--collapsed .ni.nav__pin-collapsed { display: flex; }

/* ─── Nav header button ─── */
.nhb { display: inline-flex; align-items: center; justify-content: center; padding: 8px 12px; border-radius: 12px; border: none; cursor: pointer; background: rgba(36,36,36,0); color: #242424; transition: background 0.1s; outline: none; position: relative; }
.nhb:hover { background: rgba(36,36,36,0.04); }
.nhb:focus-visible { outline: 2px solid #000; outline-offset: 0; box-shadow: inset 0 0 0 1px #fff; }
.nhb svg { display: block; width: 20px; height: 20px; }
.nhb__badge { position: absolute; top: 4px; right: 4px; min-width: 16px; height: 16px; background: #242424; border: 1px solid #242424; border-radius: 9999px; color: #fff; font-size: 10px; font-weight: 600; display: flex; align-items: center; justify-content: center; padding: 0 2px; line-height: 1; }

/* ─── Nav item ─── */
.ni { display: flex; align-items: center; gap: 8px; min-height: 36px; padding: 6px 10px 6px 12px; border-radius: 12px; width: 100%; border: none; cursor: pointer; background: rgba(36,36,36,0); font-family: inherit; font-size: 14px; font-weight: 400; line-height: 1.4; color: #242424; text-align: left; transition: background 0.1s; outline: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; position: relative; }
.ni:hover { background: rgba(36,36,36,0.04); color: #1d1d1d; }
.ni:focus-visible { outline: 2px solid #000; outline-offset: 0; box-shadow: inset 0 0 0 1px #fff; }
.ni.sel { background: #ebebeb; font-weight: 600; }
.ni.sel:hover { background: #e1e1e1; color: #1d1d1d; }
.ni svg { display: block; flex-shrink: 0; width: 20px; height: 20px; }
.ni__title { flex: 1 0 0; min-width: 0; overflow: hidden; text-overflow: ellipsis; display: flex; flex-direction: column; gap: 0; min-height: 24px; justify-content: center; }
.ni__sec { font-size: 10px; color: #6f6f6f; overflow: hidden; text-overflow: ellipsis; }
.ni.sel .ni__sec { color: #5d5d5d; }
.ni__right { display: flex; align-items: center; flex-shrink: 0; }
.ni__right-icon { display: flex; align-items: center; padding: 4px; }
.ni__right-icon svg { width: 16px; height: 16px; }
.ni__unread { width: 8px; height: 8px; border-radius: 9999px; background: #242424; flex-shrink: 0; margin-left: 4px; }

/* Collapsed — handled by .nav--collapsed parent rules above */

/* ─── Section header ─── */
.nsh { width: 100%; padding: 6px; font-size: 12px; color: #6f6f6f; cursor: default; }
.nsh__input { width: 100%; background: transparent; border: none; font: inherit; font-size: 12px; color: #6f6f6f; padding: 4px 6px; border-radius: 4px; outline: none; }
.nsh__input:focus { background: rgba(36,36,36,0.04); color: #242424; }

/* ─── Split item ─── */
.ni-split { display: flex; align-items: center; width: 100%; border-radius: 12px; }
.ni-split .ni { flex: 1 0 0; width: auto; }
.ni-split__sec { display: flex; align-items: center; justify-content: center; gap: 8px; height: 36px; padding: 6px 12px; border-radius: 12px; border: none; cursor: pointer; background: rgba(36,36,36,0); color: #242424; transition: background 0.1s; outline: none; }
.ni-split__sec:hover { background: rgba(36,36,36,0.04); }
.ni-split__sec:focus-visible { outline: 2px solid #000; outline-offset: 0; box-shadow: inset 0 0 0 1px #fff; }
.ni-split__sec svg { display: block; width: 12px; height: 12px; }

/* ─── Me control ─── */
.me { display: flex; align-items: center; gap: 8px; min-height: 36px; padding: 3px 10px; border-radius: 12px; flex: 1; min-width: 0; background: rgba(36,36,36,0); cursor: pointer; border: none; font-family: inherit; text-align: left; transition: background 0.1s; outline: none; color: #242424; }
.me:hover { background: rgba(36,36,36,0.04); }
.me__avatar { width: 28px; height: 28px; border-radius: 9999px; background: #d0d0d0; flex-shrink: 0; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; color: #fff; }
.me__info { flex: 1 0 0; min-width: 0; display: flex; flex-direction: column; padding-bottom: 1px; }
.me__name { font-size: 12px; line-height: 1.4; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.me__license { font-size: 10px; color: #6f6f6f; line-height: 1.4; display: flex; align-items: center; gap: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
`;

// ─── HTML builders ──────────────────────────────────────────

function navItem(opts: { icon?: string; label: string; secondary?: string; selected?: boolean; rightIcons?: string[]; unread?: boolean; collapsed?: boolean }): string {
  const cls = ['ni'];
  if (opts.selected) cls.push('sel');
  if (opts.collapsed) cls.push('ni--collapsed');
  let inner = '';
  if (opts.icon) inner += opts.icon;
  inner += '<div class="ni__title"><span>' + opts.label + '</span>';
  if (opts.secondary) inner += '<span class="ni__sec">' + opts.secondary + '</span>';
  inner += '</div>';
  if (opts.rightIcons || opts.unread) {
    inner += '<div class="ni__right">';
    if (opts.rightIcons) opts.rightIcons.forEach(i => { inner += '<span class="ni__right-icon">' + i + '</span>'; });
    if (opts.unread) inner += '<span class="ni__unread"></span>';
    inner += '</div>';
  }
  return '<button class="' + cls.join(' ') + '">' + inner + '</button>';
}

function splitItem(icon: string, label: string): string {
  return '<div class="ni-split"><button class="ni">' + icon + '<div class="ni__title"><span>' + label + '</span></div></button><button class="ni-split__sec">' + chevDn + '</button></div>';
}

function headerBtn(icon: string, badge?: string): string {
  return '<button class="nhb">' + icon + (badge ? '<span class="nhb__badge">' + badge + '</span>' : '') + '</button>';
}

// ─── Sections ───────────────────────────────────────────────

const body = [
  '<div class="wrap">',
  '<h1>Nav — Component Preview</h1>',
  '<p class="hint">Click the Copilot icon to toggle expanded/collapsed. Click nav items to toggle selected. Hover for hover state.</p>',

  // ─── Interactive Nav Shell ───
  '<h2>Interactive Nav Shell</h2>',
  '<div class="row">',
  '<div class="cell">',
  '<div class="nav nav--expanded" id="interactive-nav">',
  '<div class="nav__header"><button class="nhb" id="nav-toggle" title="Toggle expand/collapse">' + copilotIco + '</button><div style="flex:1"></div><button class="nhb nav__waffle">' + waffleIco + '</button></div>',
  '<div class="nav__body">',
  navItem({ icon: composeIco, label: 'New chat' }),
  navItem({ icon: searchIco, label: 'Search' }),
  navItem({ icon: agentIco, label: 'Agents' }),
  navItem({ icon: folderIco, label: 'Workspaces' }),
  '<div class="nav__section nav__section--pinned">',
  '<div class="nsh"><input class="nsh__input" value="Pinned" readonly /></div>',
  navItem({ icon: agentIco, label: 'Researcher', selected: true }),
  navItem({ icon: folderIco, label: 'Employee Training Program' }),
  navItem({ label: 'Market Analysis and Trends' }),
  '</div>',
  '<button class="ni nav__pin-collapsed" title="Pinned">' + pinIco20 + '</button>',
  '<div class="nav__section nav__section--chats">',
  '<div class="nsh"><input class="nsh__input" value="Chats" readonly /></div>',
  navItem({ label: 'Relocation benefits', rightIcons: [arrowSync16] }),
  navItem({ label: 'Escalation trends' }),
  navItem({ label: 'New capabilities in Copilot' }),
  navItem({ label: 'Latest emails from manager' }),
  navItem({ label: 'Market Analysis and Trends' }),
  navItem({ label: 'Relocation benefits', rightIcons: [arrowSync16] }),
  navItem({ label: 'Sales Forecast FY25' }),
  navItem({ label: 'Marketing Strategy for Q2' }),
  navItem({ label: 'Budget Allocation for New Proj...' }),
  '</div>',
  '</div>',
  '<div class="nav__footer"><button class="me"><span class="me__avatar">MK</span><div class="me__info"><span class="me__name">Mona Kane</span><span class="me__license">M365 Copilot (Premium)</span></div></button>' + headerBtn(alertIco, '4') + '</div>',
  '</div>',
  '</div>',
  '</div>',

  // ─── Nav Item States ───
  '<h2>Nav Item States</h2>',
  '<div class="row">',
  '<div class="cell"><span class="rl">Rest</span>' + navItem({ icon: chatIco, label: 'Nav item title' }) + '</div>',
  '<div class="cell"><span class="rl">Selected</span>' + navItem({ icon: chatIco, label: 'Nav item title', selected: true }) + '</div>',
  '<div class="cell"><span class="rl">With Status</span>' + navItem({ icon: chatIco, label: 'Nav item title', rightIcons: [arrowSync16] }) + '</div>',
  '<div class="cell"><span class="rl">Unread</span>' + navItem({ icon: chatIco, label: 'Nav item title', unread: true }) + '</div>',
  '</div>',

  // ─── Split Items ───
  '<h2>Split Items</h2>',
  '<div class="row">',
  '<div class="cell"><span class="rl">New Chat</span>' + splitItem(composeIco, 'New chat') + '</div>',
  '</div>',

  // ─── Header Buttons ───
  '<h2>Header Buttons</h2>',
  '<div class="row">',
  '<div class="cell"><span class="rl">Copilot</span>' + headerBtn(copilotIco) + '</div>',
  '<div class="cell"><span class="rl">Waffle</span>' + headerBtn(waffleIco) + '</div>',
  '<div class="cell"><span class="rl">Notification</span>' + headerBtn(alertIco, '4') + '</div>',
  '<div class="cell"><span class="rl">More</span>' + headerBtn(moreIco) + '</div>',
  '</div>',

  // ─── Me Control ───
  '<h2>Me Control</h2>',
  '<div class="row">',
  '<div class="cell"><span class="rl">Expanded</span><button class="me" style="width:282px"><span class="me__avatar">MK</span><div class="me__info"><span class="me__name">Mona Kane</span><span class="me__license">M365 Copilot (Premium)</span></div></button></div>',
  '<div class="cell"><span class="rl">Collapsed</span><button class="me me--collapsed"><span class="me__avatar">MK</span></button></div>',
  '</div>',

  '</div>',

  // ─── Script ───
  '<script>',
  '  // Toggle expand/collapse on Copilot icon click',
  '  document.getElementById("nav-toggle").addEventListener("click", function(e) {',
  '    e.stopPropagation();',
  '    var nav = document.getElementById("interactive-nav");',
  '    nav.classList.toggle("nav--expanded");',
  '    nav.classList.toggle("nav--collapsed");',
  '  });',
  '  // Single selection on nav item click',
  '  document.querySelectorAll(".ni").forEach(function(el) {',
  '    el.addEventListener("click", function() {',
  '      document.querySelectorAll(".ni.sel").forEach(function(s) { s.classList.remove("sel"); });',
  '      el.classList.add("sel");',
  '    });',
  '  });',
  '</script>',
].join('\n');

const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/>'
  + '<meta name="viewport" content="width=device-width,initial-scale=1"/>'
  + '<title>Nav Preview</title>'
  + '<style>' + css + '</style></head><body>'
  + body + '</body></html>';

const outDir = path.join(path.dirname(new URL(import.meta.url).pathname), '..', 'dist');
if (!fs.existsSync(outDir)) { fs.mkdirSync(outDir, { recursive: true }); }
const outPath = path.join(outDir, 'nav.html');
fs.writeFileSync(outPath, html, 'utf-8');
console.log('Done: ' + outPath);
