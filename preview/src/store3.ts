/**
 * Store Model 3 — Agents in Store, Skills + Sources in Settings
 *
 * Renders the Copilot shell with:
 *   - Store view: Agent-only catalog (no tabs, cleanest store)
 *   - Settings panel: Skills as "Capability Packs" + Sources as "Connected Apps"
 *
 * Usage:  npx tsx preview/src/store3.ts
 * Output: preview/dist/store3.html
 */
...
const composeIco = '<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M10.5 4.00003C10.7761 4.00003 11 4.22389 11 4.50003C10.9999 4.77614 10.7761 5.00003 10.5 5.00003H6C4.89543 5.00003 4 5.89546 4 7.00002V14C4.00004 15.1045 4.89545 16 6 16H13C14.1045 16 14.9999 15.1045 15 14V9.50002ZM16.1465 3.14652C16.3417 2.95126 16.6582 2.95126 16.8535 3.14652C17.0487 3.34179 17.0487 3.65832 16.8535 3.85355L9.06054 11.6455L7.99999 12L8.35351 10.9395L16.1465 3.14652Z" fill="currentColor"/></svg>';
...
// Model banner
css += '.model-banner { background: linear-gradient(135deg, #E8DEF8 0%, #D4EDDA 100%); padding: 12px 32px; font-size: 13px; font-weight: 600; color: #424242; display: flex; align-items: center; gap: 8px; flex-shrink: 0; }';
css += '\n';
...
// Store header — cleaner, agents-only
...
// Cards — only agents, no badges needed
...
css += '\n';
css += '.card-lg__name { font-size: 14px; font-weight: 600; line-height: 1.3; }';
css += '\n';
...
css += '\n';
css += '.card-lg__rec { margin-top: 6px; font-size: 11px; color: #6f6f6f; display: flex; align-items: center; gap: 4px; }';
css += '\n';
css += '.card-lg__rec-dot { width: 6px; height: 6px; border-radius: 3px; }';
css += '\n';

// Settings panel — wider, with two sections
css += '.settings-panel { display: none; position: absolute; top: 0; right: 0; bottom: 0; width: 480px; background: #fff; box-shadow: -4px 0 24px rgba(0,0,0,0.1); z-index: 500; flex-direction: column; border-left: 1px solid #f0f0f0; }';
css += '\n';
...
css += '\n';
css += '.settings-panel__close { width: 32px; height: 32px; border-radius: 8px; border: none; background: transparent; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #5d5d5d; }';
css += '\n';
...
// Settings nav tabs (within panel)
css += '.settings-nav { display: flex; gap: 0; border-bottom: 1px solid #f0f0f0; padding: 0 24px; }';
css += '\n';
css += '.settings-nav__tab { padding: 10px 16px; font-size: 13px; font-weight: 400; color: #5d5d5d; cursor: pointer; border: none; background: none; font-family: inherit; border-bottom: 2px solid transparent; transition: color 0.15s, border-color 0.15s; }';
css += '\n';
css += '.settings-nav__tab:hover { color: #242424; }';
css += '\n';
css += '.settings-nav__tab--active { color: #242424; font-weight: 600; border-bottom-color: #242424; }';
css += '\n';
...
css += '.settings-content { display: none; }';
css += '\n';
css += '.settings-content--active { display: block; }';
css += '\n';

// Source/skill row
css += '.settings-row { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 12px; border: 1px solid #f0f0f0; margin-bottom: 8px; transition: background 0.1s; cursor: pointer; }';
css += '\n';
css += '.settings-row:hover { background: #fafafa; }';
css += '\n';
css += '.settings-row__icon { width: 32px; height: 32px; flex-shrink: 0; }';
css += '\n';
css += '.settings-row__icon svg { display: block; }';
css += '\n';
css += '.settings-row__info { flex: 1; min-width: 0; }';
css += '\n';
css += '.settings-row__name { font-size: 14px; font-weight: 500; }';
css += '\n';
css += '.settings-row__desc { font-size: 12px; color: #6f6f6f; margin-top: 2px; }';
css += '\n';
...
css += '\n';
css += '.settings-row__chevron { width: 20px; height: 20px; color: #929292; flex-shrink: 0; }';
css += '\n';
css += '.settings-row__chevron svg { width: 20px; height: 20px; }';
css += '\n';
...
css += '\n';
css += '.modal__rec-apps { margin-top: 16px; padding: 12px 16px; border-radius: 10px; background: #F0F7FF; border: 1px solid #B3D7FF; font-size: 13px; color: #0050A0; line-height: 1.5; }';
css += '\n';
...
html += '<title>Store Model 3 \u2014 Agents Store Only</title>';
...
html += '<a class="ni sel" href="store3.html">' + storeIco + '<span class="ni__title">Store 3: Agents</span></a>';
...
// Main
html += '<div class="main">';
html += '<div class="model-banner"><span class="model-banner__label">MODEL 3</span> Agents Store Only \u2014 Skills + Sources bundled as Apps in Settings</div>';

// Header — NO tabs, cleaner
html += '<div class="store-hdr"><span class="store-hdr__title">Agents</span>';
html += '<div class="store-hdr__actions">';
html += '<button class="store-hdr__btn" id="openSettings">' + settingsIco + ' Apps & Capabilities</button>';
html += '<button class="store-hdr__btn" style="background:#242424;color:#fff;border-color:#242424;">' + addIco + ' Create agent</button>';
html += '</div></div>';

// No tabs — agents only
html += '<div class="store-content">';
html += '<div class="store-search"><span>' + searchIco + '</span><input type="text" placeholder="Search for an agent" /></div>';

// My agents
html += '<div class="store-section"><div class="store-section__title">My agents</div><div class="card-grid">';

const myAgents = [
  { n: 'Sales', c: '#0078D4', l: 'Sa', d: 'Drive deals forward with clear insights and ready-to-use actions.' },
  { n: 'Idea Coach', c: '#FFB800', l: 'IC', d: 'Creative brainstorming and idea generation partner.' },
  { n: 'Analyst', c: '#F2994A', l: 'An', d: 'Turn complex data into quick, easy-to-read answers.' },
  { n: 'Visual Creator', c: '#E040A0', l: 'VC', d: 'Create stunning visuals with Microsoft 365 Copilot.' },
  { n: 'Researcher', c: '#6B4FBB', l: 'Re', d: 'Fast insights to prep for your next meeting.' },
  { n: 'Power BI', c: '#F2C800', l: 'PB', d: 'Interactive data dashboards and business analytics.' },
  { n: 'Brainstormer', c: '#00BFA5', l: 'Br', d: 'Group ideation and creative thinking sessions.' },
  { n: 'People', c: '#4A90D9', l: 'Pe', d: 'Find colleagues, org charts, and team information.' },
];

for (const a of myAgents) {
  html += `<div class="card-compact" data-name="${a.n}" data-desc="${a.d}" data-creator="Microsoft" data-color="${a.c}">`;
  html += `<span class="card-compact__icon">${ico(a.c, a.l)}</span>`;
  html += `<span class="card-compact__name">${a.n}</span>`;
...
const msAgents = [
  { n: 'Sales', d: 'Drive deals forward with clear insights and ready-to-use actions.', c: '#0078D4', l: 'Sa', rec: 'Salesforce' },
  { n: 'Planner', d: 'All your tasks and projects in one simple, familiar experience.', c: '#00A651', l: 'Pl', rec: '' },
  { n: 'Researcher', d: 'Fast insights to prep for your next meeting.', c: '#6B4FBB', l: 'Re', rec: '' },
  { n: 'Analyst', d: 'Turn complex data into quick, easy-to-read answers.', c: '#F2994A', l: 'An', rec: '' },
  { n: 'Employee Self Service', d: 'Assists employees with responses based on official policies.', c: '#0078D4', l: 'ES', rec: 'ServiceNow' },
  { n: 'RFP Generator', d: 'Build an RFP like a Microsoft Pro.', c: '#FF5733', l: 'RG', rec: '' },
];

for (const a of msAgents) {
  html += `<div class="card-lg" data-name="${a.n}" data-desc="${a.d}" data-creator="Microsoft" data-color="${a.c}">`;
  html += `<span class="card-lg__icon">${ico(a.c, a.l)}</span>`;
  html += '<div class="card-lg__text">';
  html += `<span class="card-lg__name">${a.n}</span>`;
  html += `<span class="card-lg__desc">${a.d}</span>`;
  if (a.rec) {
    html += `<span class="card-lg__rec"><span class="card-lg__rec-dot" style="background:${a.c}"></span>Works best with: ${a.rec}</span>`;
  }
  html += '</div></div>';
}
html += '</div></div>';
...
// Built by your org
html += '<div class="store-section"><div class="store-section__title">Built by your org</div><div class="card-grid--lg">';

const orgAgents = [
  { n: 'Onboarding Buddy', d: 'Helps new employees get set up and find their way around.', c: '#4CAF50', l: 'OB' },
  { n: 'IT Help Desk', d: 'Troubleshoot common IT issues and submit tickets.', c: '#607D8B', l: 'IT' },
  { n: 'Brand Guardian', d: 'Enforce brand guidelines and review content for consistency.', c: '#E91E63', l: 'BG' },
];

for (const a of orgAgents) {
  html += `<div class="card-lg" data-name="${a.n}" data-desc="${a.d}" data-creator="Your org" data-color="${a.c}">`;
  html += `<span class="card-lg__icon">${ico(a.c, a.l)}</span>`;
  html += `<div class="card-lg__text"><span class="card-lg__name">${a.n}</span><span class="card-lg__desc">${a.d}</span></div></div>`;
}
html += '</div></div>';

html += '</div>'; // store-content

// ─── Settings Panel (two tabs: Connected Apps + Capabilities) ───
html += '<div class="settings-panel" id="settingsPanel">';
html += '<div class="settings-panel__hdr"><span class="settings-panel__title">Apps & Capabilities</span>';
html += '<button class="settings-panel__close" id="closeSettings">' + dismissIco + '</button></div>';

html += '<div class="settings-nav">';
html += '<button class="settings-nav__tab settings-nav__tab--active" data-stab="apps">Connected Apps</button>';
html += '<button class="settings-nav__tab" data-stab="caps">Capabilities</button>';
html += '</div>';

html += '<div class="settings-panel__body">';

// Connected Apps tab
html += '<div class="settings-content settings-content--active" id="sApps">';
html += '<div class="settings-panel__desc">Connected apps provide data and actions to all agents. Each app bundles its data access with source-specific skills.</div>';

html += '<div><div class="settings-panel__section-title">Connected</div>';

const connectedApps = [
  { n: 'Salesforce', c: '#00A1E0', l: 'SF', d: 'CRM data + create leads, log calls', on: true },
  { n: 'Jira', c: '#0052CC', l: 'Ji', d: 'Tickets, sprints + create issues', on: true },
  { n: 'GitHub', c: '#24292E', l: 'GH', d: 'Repos, PRs + create issues', on: false },
];

for (const s of connectedApps) {
  html += `<div class="settings-row"><span class="settings-row__icon">${ico(s.c, s.l)}</span>`;
  html += `<div class="settings-row__info"><div class="settings-row__name">${s.n}</div><div class="settings-row__desc">${s.d}</div></div>`;
  html += `<div class="toggle ${s.on ? 'toggle--on' : ''}"><div class="toggle__knob"></div></div>`;
  html += `<span class="settings-row__chevron">${chevronRightIco}</span></div>`;
}
html += '</div>';
...
// Capabilities tab
html += '<div class="settings-content" id="sCaps">';
html += '<div class="settings-panel__desc">Standalone capabilities available to all agents. These are generic skills not tied to a specific data source.</div>';

const capabilities = [
  { n: 'Generate Report', c: '#0F7B3F', l: 'GR', d: 'Create polished reports from data and insights', on: true },
  { n: 'Summarize', c: '#00A651', l: 'Su', d: 'Condense documents and threads into key points', on: true },
  { n: 'Web Search', c: '#4A90D9', l: 'WS', d: 'Deep web search with citations and verification', on: true },
  { n: 'Data Visualize', c: '#0078D4', l: 'DV', d: 'Create charts and graphs from datasets', on: true },
  { n: 'Translate', c: '#FF9800', l: 'Tr', d: 'Translate content across 100+ languages', on: false },
  { n: 'Image Generate', c: '#E040A0', l: 'IG', d: 'Generate images from text descriptions', on: true },
];

for (const c of capabilities) {
  html += `<div class="settings-row"><span class="settings-row__icon">${ico(c.c, c.l)}</span>`;
  html += `<div class="settings-row__info"><div class="settings-row__name">${c.n}</div><div class="settings-row__desc">${c.d}</div></div>`;
  html += `<div class="toggle ${c.on ? 'toggle--on' : ''}"><div class="toggle__knob"></div></div></div>`;
}

html += '</div>';
html += '</div></div>'; // settings body + panel
...
html += '<div class="modal__rec-apps" id="modalRec" style="display:none"></div>';
html += '</div></div></div>';
...
// Settings panel
html += 'document.getElementById("openSettings").addEventListener("click", function() { document.getElementById("settingsPanel").classList.add("settings-panel--open"); });';
html += '\n';
html += 'document.getElementById("closeSettings").addEventListener("click", function() { document.getElementById("settingsPanel").classList.remove("settings-panel--open"); });';
html += '\n';

// Settings tabs
html += 'document.querySelectorAll(".settings-nav__tab").forEach(function(tab) {';
html += '  tab.addEventListener("click", function() {';
html += '    document.querySelectorAll(".settings-nav__tab").forEach(function(t) { t.classList.remove("settings-nav__tab--active"); });';
html += '    tab.classList.add("settings-nav__tab--active");';
html += '    var target = tab.getAttribute("data-stab");';
html += '    document.querySelectorAll(".settings-content").forEach(function(c) { c.classList.remove("settings-content--active"); });';
html += '    document.getElementById(target === "apps" ? "sApps" : "sCaps").classList.add("settings-content--active");';
html += '  });';
html += '});';
html += '\n';

// Toggle switches
html += 'document.querySelectorAll(".toggle").forEach(function(t) { t.addEventListener("click", function() { t.classList.toggle("toggle--on"); }); });';
html += '\n';
...
html += '    document.getElementById("modalDesc").innerHTML = "<p>" + desc + "</p><p>This agent provides specialized expertise directly within Copilot, helping you work more efficiently.</p>";';
...
html += '    var rec = document.getElementById("modalRec");';
html += '    if (name === "Sales") {';
html += '      rec.style.display = "block";';
html += '      rec.innerHTML = \'<strong>Recommended apps:</strong> This agent works best with <strong>Salesforce</strong> connected. <a href="#" onclick="document.getElementById(\\\'modalOverlay\\\').classList.remove(\\\'modal-overlay--open\\\');document.getElementById(\\\'settingsPanel\\\').classList.add(\\\'settings-panel--open\\\');return false;" style="color:#0050A0">Set up in Apps \u2192</a>\';';
html += '    } else if (name === "Employee Self Service") {';
html += '      rec.style.display = "block";';
html += '      rec.innerHTML = \'<strong>Recommended apps:</strong> Works best with <strong>ServiceNow</strong> connected.\';';
html += '    } else { rec.style.display = "none"; }';
...
fs.writeFileSync(path.join(outDir, 'store3.html'), html, 'utf-8');
console.log('Done: preview/dist/store3.html');
