/**
  * ConnectorSkills P0 — Experiment (cloned from Connectors & Skills v3)
 *
...
function zeroState(): string {
  ...
  const removeIcon = '<svg viewBox="0 0 12 12" width="12" height="12" fill="none"><path d="M3 3l6 6M9 3l-6 6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>';
  const portfolioAttachment = '<div class="cmp-attach" data-file="John Portfolio FY26" title="John Portfolio FY26.xlsx">'
    + '<span class="cmp-attach__ico">' + brandSvg('excel-app-20-color') + '</span>'
    + '<span class="cmp-attach__meta"><span class="cmp-attach__name">John Portfolio FY26</span></span>'
    + '<button class="cmp-attach__x" aria-label="Remove John Portfolio FY26">' + removeIcon + '</button>'
    + '</div>';
  return [
    '<div class="zs">',
    '<div class="zs__greeting">Hi Elvia, how can I help?</div>',
    '<div class="thread" id="thread"></div>',
    '<div class="cmp ci--has-text" id="cmp">',
    addMenuMarkup(),
    '<textarea class="cmp__ta" id="cmpTa" rows="1" placeholder="Message Copilot">Help me analyse my client\u2019s investment portfolio against current market trends. Identify the stocks to hold or sell.</textarea>',
    '<div class="cmp__row">',
    '<button class="cmp__btn" id="addBtn" title="Add" aria-haspopup="menu" aria-expanded="false">' + ico('add-20-regular') + '</button>',
    '<div class="cmp__attach" id="cmpAttach">' + portfolioAttachment + '</div>',
...
const html = '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>ConnectorSkills P0</title><style>' + css + agentsCss() + '</style></head><body>'
  + '<div class="shell">'
  + nav()
  + '<div class="main">' + header() + '<div class="body">' + zeroState() + '</div><div class="disc">Copilot is an AI and may make mistakes. Using Copilot means you agree to the Terms of Use. See our Privacy Statement.</div>' + agentsView() + '</div>'
  + cotPanel()
  + '</div>'
  + promptLab()
  + sourcesModal()
...
fs.writeFileSync(path.join(outDir, 'connectorSkillsP0.html'), html, 'utf-8');
console.log('Done: ' + path.join(outDir, 'connectorSkillsP0.html'));
