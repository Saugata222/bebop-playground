/**
  * User Sync Experiment — copied from Discovery v2
 *
 * Focuses on the user-sync experience: clearer sync loader plus explicit
 * connecting / disconnecting states with hover-and-focus status tooltips.
 * Source-menu and settings "Recommended"/"New" pill tags are removed; Confluence
 * is added to the recommended list.
 *
...
interface SrcConn { logo: string; name: string; meta?: string; desc: string; trailing: 'on' | 'off' | 'connect' | 'chevron'; tenant?: boolean; cat: string; badge?: boolean; usc?: boolean; uscInput?: boolean; uscHost?: string; uscDev?: string; uscAdd?: string; uscPh?: string; }
const SRC_CONNECTORS: SrcConn[] = [
  ...
  { logo: 'confluence-logo.png', name: 'Confluence', desc: 'Pull knowledge base articles and team documentation', trailing: 'connect', cat: 'Development tools', usc: true, uscInput: true, uscHost: 'atlassian.net', uscDev: 'Microsoft' },
  ...
  { logo: 'google-drive-logo.png', name: 'Google Drive', desc: 'Search documents, sheets, and slides in Drive', trailing: 'connect', cat: 'Other', usc: true, uscDev: 'Microsoft' },
];
...
const SRC_REC = ['London Stock Exchange Group', 'Moody\u2019s', 'HubSpot', 'Confluence'];
...
const SRC_MODEL = SRC_CONNECTORS.map(function (c) {
  return { n: c.name, logo: srcLogoHtml(c.logo), stgLogo: stgLogoHtml(c.logo), logoSrc: c.logo.endsWith('.svg') ? '' : logoImg(c.logo), meta: c.meta || '', desc: c.desc, state: c.trailing, cats: c.cat ? [c.cat] : [], rec: SRC_REC.indexOf(c.name) !== -1, tenant: c.tenant === true, badge: c.badge === true, usc: c.usc === true, uscInput: c.uscInput === true, uscHost: c.uscHost || '', uscDev: c.uscDev || '', uscAdd: c.uscAdd || '', uscPh: c.uscPh || '' };
});
...
function authModal(): string {
  return [
    ...
    '<div class="ad__usc-item"><div class="ad__usc-t">You\u2019re in control</div><div class="ad__usc-b">Copilot is strictly limited to the permissions you have explicitly set. You can disable access anytime. <a class="ad__link" href="#" onclick="return false">Learn more</a></div></div>',
    '<div class="ad__usc-div" id="adUscWsDiv"></div>',
    '<div class="ad__usc-item" id="adUscWs"><div class="ad__usc-t">Add your organisation\u2019s workspace</div><div class="ad__usc-b" id="adUscAdd">Paste any url from your workspace to connect</div>',
...
.ad__usc-item[hidden] { display: none; }
.ad__usc-div[hidden] { display: none; }
...
const js = [
  ...
  "  function stgRowHtml(item){ var isConnect = item.state === 'connect'; var isSync = item.state === 'syncing' || item.state === 'removing'; var trailing = isConnect ? '<button class=\"stg__connect\">Connect</button>' : (isSync ? syncTrailing(item) : '<span class=\"stg__chev\">' + STG_CHEV + '</span>'); var tag = (isConnect || isSync) ? 'div' : 'button'; return '<' + tag + ' class=\"stg__row\" data-name=\"' + item.n + '\"><span class=\"stg__rowicon\">' + item.stgLogo + '</span><span class=\"stg__rowmeta\"><span class=\"stg__namerow\"><span class=\"stg__rowname\">' + item.n + '</span></span><span class=\"stg__rowsub\">' + item.desc + '</span></span>' + trailing + '</' + tag + '>'; }",
  ...
  "  function srcRowHtml(item){ var meta = item.meta ? '<span class=\"srcm__meta\">' + item.meta + '</span>' : ''; return '<div class=\"srcm__row\" data-name=\"' + item.n + '\"><div class=\"srcm__inner\">' + item.logo + '<span class=\"srcm__label\">' + item.n + '</span><span class=\"srcm__spacer\"></span>' + meta + srcTrailing(item) + '</div></div>'; }",
...
fs.writeFileSync(path.join(outDir, 'userSyncExperiment.html'), html, 'utf-8');
console.log('Done: ' + path.join(outDir, 'userSyncExperiment.html'));
